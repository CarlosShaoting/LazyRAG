...
- 发布 13 个 `browser.*` Tool；不提供任意 JavaScript 或任意 CDP passthrough。
...
- 复用相同 13 个 MCP Tool 和 WebSocket 命令协议，支持 `open/navigate/snapshot/click/type/select/press/scroll/wait/screenshot/tabs/close`。
...
- Browser MCP：Bearer token、13 个 Tool schema 发布。
...
10. **复杂页面**：跨域 iframe、页面新开的 popup/tab、富文本编辑器、Canvas、文件上传尚未支持。
11. **截图对象存储**：截图当前以内联 JPEG base64 返回，尚未改为短期对象 URL。
