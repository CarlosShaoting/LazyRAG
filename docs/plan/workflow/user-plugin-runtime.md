# 用户 Workflow 接入 ChatAgent、RemoteFS 版本存储与默认启停方案

## 1. 背景与目标

当前内置 Workflow 由 Chat 服务启动时从固定目录扫描并完整加载到进程级 `_registry`。ChatAgent 冷启动时读取各 Workflow 的 `description` 和 `when_to_use`，注册 trigger tool；模型选择 Workflow 后，再进入完整的 `scenario.md`、状态机、SubAgent 和 DriverAgent 执行流程。

用户创建或由 Skill 转换得到的 Workflow 目前只保存在 `plugin_drafts`，没有进入 ChatAgent 的运行时发现链路；现有配置也只有 `enable_workflow` 总开关，不能分别配置内置和用户 Workflow 的默认启停状态。

本方案实现：

1. 用户 Workflow 必须显式发布，草稿不会直接影响 ChatAgent。
2. Workflow 复用 Skill 已有的 RemoteFS、VersionFS 和按 hash 去重的 Blob 存储。
3. 新发布的用户 Workflow 默认关闭；用户开启后才成为 ChatAgent 候选。
4. 冷启动只向模型提供轻量说明、`when_to_use` 和 trigger tool。
5. 模型调用 trigger 后，Chat 才通过 RemoteFS 读取并物化指定 revision 的完整 Workflow。
...
新增独立 Workflow namespace，不将 Workflow 混入 `remote://skills`：

```text
remote://workflows/<owner-scope>/<workflow-id>/
├── workflow.yaml
├── scenario/
│   ├── scenario.md
│   ├── state.yml
...
```text
remote://workflows/builtin/image-workflow
remote://workflows/u_a81f2c/paper-review
```

`owner-scope` 使用稳定的服务端标识，不接受客户端任意拼接。Workflow 的真实身份为 `plugin_ref`：

```text
builtin:<workflow-id>
user:<owner-user-id>:<workflow-id>
```
...
本期不将 revision entries 改成父链差异。Workflow 文件数量少，差异链会显著增加历史版本读取和容错复杂度，存储收益很小。

### 2.4 Skill 与 Workflow 的关系

Skill 与 Workflow 是两个独立业务域，不共用资源记录、业务表、Blob 表或权限模型，只复用文件版本管理代码和 RemoteFS 协议：
...
从 Skill 转换得到的 Workflow 仅保留 `source_skill_id` 用于来源追溯。转换时文件内容从 Skill 包复制到 Workflow draft，此后两者独立演进，不建立运行时依赖。

### 2.5 Core 与 Chat 的职责

Core 负责发布、版本、权限、启停、Catalog 和 RemoteFS revision view。Chat 不直接查询 Workflow 数据表，也不增加 Workflow 专属 runtime-bundle API。

RemoteFS 底层仍通过现有 `/remote-fs/*` Core 接口读取；这里避免的是新增算法到后端的 Workflow 专属内容 API，并统一复用文件系统访问机制。
...
relative_root            -- workflows/<owner-scope>/<workflow-id>
name
description
when_to_use
head_revision_id
...
`description` 和 `when_to_use` 作为可检索的发布元数据，在 commit 后从 head revision 的 `workflow.yaml` 提取并同步，供 Catalog 使用。

### 3.2 Workflow VersionStore

第一期不迁移现有 Skill 表，复用通用 `versionfs.Engine` 接口，为 Workflow 实现薄适配器：
...
Skill 和 Workflow 的 Blob 只在各自业务域内按 hash 去重，不进行跨域去重。这样可以保持权限、GC、迁移和故障域完全独立；本方案不引入 `versioned_resources` 或共享 `resource_blobs` 表。
...
```text
effective_enabled =
  user_chat_settings.enable_workflow
  AND workflows.status = active
  AND 当前用户有权使用该 Workflow
  AND user_plugin_settings.enabled
```

默认值：

- 现有内置 Workflow 迁移后默认开启。
- 新增内置 Workflow 可由 manifest 的 `default_enabled` 提供初始值。
- 新发布或从 Skill 转换的用户 Workflow 默认关闭。
- 本期只支持用户级默认配置，不提供会话级逐 Workflow 覆盖。
...
Workflow adapter 负责将 `workflows/<owner-scope>/<workflow-id>` 解析为 Workflow 资源 ID、校验权限，并调用 Workflow VersionStore。

通用层通过 namespace registry 路由，不直接引用 `skills` 或 `workflows` 表：
...
registry.Register("workflows", pluginAdapter)
```
...
- Workflow adapter：解析 owner-scope/workflow-id、Workflow 权限、`plugin_*` 表和 Workflow Blob Store。
- RemoteFS 不理解 `SKILL.md` 或 `workflow.yaml` 的业务规则；这些规则分别留在 Skill Service 和 Workflow Service。
...
```http
GET /remote-fs/list?path=workflows/...&revision_id=<uuid>
GET /remote-fs/info?path=workflows/...&revision_id=<uuid>
GET /remote-fs/exists?path=workflows/...&revision_id=<uuid>
GET /remote-fs/content?path=workflows/...&revision_id=<uuid>&encoding=raw
```
...
backend/core/workflow/remotefs/           Workflow adapter
...
```http
POST /api/workflow-drafts/{draft_id}:publish
```

发布过程：

1. 将现有 Workflow 草稿内容映射为 RemoteFS package draft；过渡期保留原 `plugin_drafts` 作者界面数据。
2. 校验 `workflow.yaml`、`state.yml`、`scenario.md`、transitions、steps、slots 和 UI 引用。
3. 校验工具白名单与 scripts 权限。
4. 通过 `versionfs.Engine.CommitDraft` 创建不可变 revision。
5. 从新 head 的 `workflow.yaml` 同步 `name/description/when_to_use/contains_scripts`。
6. 首次发布创建默认关闭的 `user_plugin_settings`。

重复发布生成新 revision，保持用户启停状态不变。删除草稿不删除已发布 revision；停止后续使用通过 archive 完成。

普通用户不得发布 `tool_scripts` 或任意 Python scripts；管理员可以发布含 scripts 的 Workflow。

### 5.2 默认启停接口

```http
GET /api/chat/settings/workflows
PATCH /api/chat/settings/workflows/{encoded_plugin_ref}
```
...
GET 统一返回内置和用户 Workflow，包括 `plugin_ref`、来源、head revision、状态和当前用户 enabled 值。
    ...
    "catalog": [
      {
        ...
        "source_type": "skill",
        "remote_root": "remote://workflows/u_a81f2c/paper-review",
...
总开关关闭时 Catalog 为空。未启用、无权限、无 head revision、已归档或 revoked 的 Workflow 不得进入 Catalog。
...
- `_trigger_workflow_step`、advance tools 和查询工具。
...
2. 内置 Workflow 可从静态 registry 返回；上层仍使用相同 RuntimeKey。
3. 用户 Workflow 调用 `RemoteFS.materialize_dir(..., revision_id=...)`。
4. 物化到 `<runtime-cache>/<workflow-ref-hash>/<revision-id>/` 临时目录。
5. 校验物化文件树 hash 与 Catalog 的 `tree_hash` 一致。
6. 原子 rename 后构造 `PluginSpec`。
7. 成功后写入 LRU 缓存；失败时清理临时目录且不污染全局 registry。

用户 Workflow 不写入进程级 `_registry`，避免同名覆盖和跨用户串用。
...
_trigger_workflow_step(execution_context, first_steps[0], user_input, is_cold_start=True)
```

完整 `scenario.md` 仍仅在 Workflow Session 启动后注入。
...
`get_plugin/get_state_machine/get_step_config/get_scenario/get_driver` 改为基于执行上下文解析。保留字符串兼容层，将旧 `image-workflow` 映射为 `builtin:image-workflow`，分阶段迁移现有调用点。

活跃 Session 恢复时从 `plugin_context` 读取固定 revision，先调用 resolver，再执行现有 scenario、state machine、driver 和 advance 逻辑。

## 8. 前端与权限

Workflow 管理列表统一展示内置和用户 Workflow，并增加用户默认 Switch：

- 用户 Workflow 列表显示是否已发布以及当前 head revision 版本号。
- 每个用户 Workflow 只有一份可变草稿；`(created_by, plugin_id)` 唯一约束继续生效。
- 详情页默认加载草稿，可切换查看任意历史 revision；历史 revision 始终只读。
- 用户点击“编辑此版本”并确认后，用选定 revision 的完整文件树覆盖唯一草稿，不修改任何已发布 revision。
- 用户 Workflow 首次发布后默认关闭。
- 详情页展示草稿状态、head revision、版本历史、来源和启停状态。
- 提供发布新 revision、回滚和归档操作。
- 总开关关闭时保留逐项配置，但 ChatAgent 不获得任何 Workflow。
...
1. 为 Workflow 增加资源、Blob、revision、entry、draft 和 draft overlay 表，并实现 `PluginVersionStore`。
2. 将 RemoteFS handler 抽出通用 namespace registry 和 adapter 接口，现有 Skill 逻辑迁入 Skill adapter，保持外部行为不变。
3. 增加 `workflows/...` namespace 和指定 `revision_id` 的只读视图。
4. 扩展 Python `RemoteFS` 的 revision 参数和 `materialize_dir`。
5. 将现有 Workflow 草稿接入 RemoteFS draft 与 VersionFS commit 发布。
6. 增加用户逐 Workflow 设置和 Core Catalog 解析。
...
10. 上线前端发布、回滚、归档和逐 Workflow Switch。
11. 通过 feature flag 先验证内置 Workflow，再开放用户 Workflow。
...
- Skill 和 Workflow 分别写入 `skill_blobs` 与 `plugin_blobs`，不存在跨域引用或 GC 影响。
- 每个 revision 的完整 tree manifest 可独立读取并产生稳定 tree hash。
- 指定 revision 的 list/info/content/materialize 始终读取同一不可变版本。
- revision 不属于目标 Workflow、越权 owner-scope 或 revision view 写操作均被拒绝。
...
- 新用户 Workflow 默认关闭，开启后才进入下一次聊天请求的 Catalog。
- 总开关关闭时 Catalog 为空。
- 用户不能读取、启用或执行其他用户的私有 Workflow。

### 10.3 渐进加载与版本固定

- 冷启动只包含轻量 Catalog，不物化完整 Workflow。
- 未调用 trigger 时不读取 state、scenario 或 scripts。
- trigger 后才 materialize 指定 revision，并加载首个 step。
- 不同用户同名 Workflow 的工具名、缓存、目录和 Session 相互隔离。
- 新 Session 使用最新 head revision，旧 Session 继续使用固定旧 revision。
- tree hash 不一致、revision 缺失或权限失效时拒绝启动且不污染缓存。

### 10.4 回归

- 内置 image/writer Workflow 的触发、dynamic/auto、advance、rewind、retry 和 DriverAgent 行为不变。
- `enable_workflow=false` 仍回退到纯 QA。
...
| Workflow RemoteFS adapter | Workflow 路径、权限及 `plugin_*` 表访问 |
| Core Workflow | 草稿、发布、元数据、启停和轻量 Catalog |
| Chat 请求 | 携带当前用户已启用 Workflow 的轻量 Catalog |
| Cold-start ChatAgent | 读取说明与 `when_to_use`，暴露轻量 trigger |
| PluginResolver | trigger 后通过 RemoteFS 物化、校验并缓存完整 Workflow |
| PluginLoader / PluginSpec | 继续解析目录、状态机、scenario、driver 和 scripts |
| PluginManager | 继续负责 trigger、advance、rewind、Session 和任务执行 |

Skill 和 Workflow 各自维护完整独立的数据表与业务逻辑，只共享 VersionFS 引擎、RemoteFS 协议与通用实现。核心改造集中在 RemoteFS namespace、指定 revision 读取、Workflow VersionStore、Workflow 身份和 `PluginSpec` 查找层；现有状态机、SubAgent、DriverAgent、artifact 和工作流执行机制继续复用。
