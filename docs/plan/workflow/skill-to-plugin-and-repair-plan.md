# Skill → Workflow 生成与 Workflow Repair 完整实施计划

## 1. 背景与目标

LazyMind 已具备 AI 辅助创建 Workflow、从 Skill 导入生成 Workflow，以及对 Workflow 状态机、UI、场景进行 AI Repair 的能力。随后 Skill 被重构为支持版本管理和文件夹的 Skill v2；Workflow 也已经增加了基于 revision、blob、文件树和 tree hash 的版本管理。
...
6. 让 Workflow Repair 与新的 Workflow revision 体系、Skill 来源 revision、覆盖证据和脚本安全规则保持一致。

本文档不重新设计 Workflow 自身的版本管理；现有 Workflow revision 能力作为既有基础直接复用。
...
- Workflow resource、blob、revision、revision entry、head revision 和 tree hash。
- Workflow draft 的 `base_revision_id`，用于表示 draft 基于哪个已发布 Workflow revision。
- 发布时将 `workflow.yaml`、`scenario/state.yml`、`scenario/scenario.md` 和 `scripts/*` 组成不可变文件树。
- Workflow 版本列表、历史版本查看、用历史版本覆盖 draft、rollback。
- Workflow session 固定 `plugin_revision_id`、`plugin_revision_no`、`plugin_tree_hash` 和 `plugin_remote_root`。
- 算法运行时按 `(plugin_ref, revision_id, tree_hash)` 物化并缓存不可变 Workflow revision，同时校验 tree hash。
...
- Repair 只根据当前 Workflow YAML/state 和用户提示修复，没有来源 Skill revision、证据覆盖和脚本映射上下文。
...
- `base_revision_id`：当前 Workflow draft 基于哪个已发布 Workflow revision。
- `source_skill_revision_id`：该 Workflow 生成或重新分析时使用的来源 Skill revision。

二者语义不同，必须分别存储。Workflow rollback 不改变来源 Skill revision；Skill 更新也不能自动改变 Workflow draft 的 base revision。

### 3.2 当前生效 Skill 版本

“当前生效版本”固定定义为请求开始时的 Skill `head_revision_id`：

- 未提交 Skill draft 不参与 Workflow 生成。
- 生成任务入队前固定 revision ID、revision number 和 tree hash。
- 任务排队或执行期间即使 Skill head 更新，当前任务仍使用已固定的 revision。
- 用户若希望使用新的 Skill head，必须显式发起“基于最新 Skill 重新分析/更新 Workflow”。
...
- `script_reused`：脚本或函数被 Workflow 复用。
...
AI 生成和 AI Repair 都只能修改 Workflow draft：

- 已发布 Workflow revision 永远不可原地修改。
- Repair 成功后仍是 draft 变更；用户发布时创建新的 Workflow revision。
- 运行中的 Workflow session 继续使用它启动时固定的 revision，不受 repair 或新发布影响。

---

## 4. 数据模型

### 4.1 Workflow draft 来源字段
...
新增 `plugin_generation_analyses`，避免把大量中间结果塞入 Workflow revision 文件树：
...
- 对应 Skill 脚本标记为 `tool_replaced`，不复制到 Workflow。
...
- 保留原函数实现，避免无依据重写。
- 复制到生成后的 Workflow `scripts/`。
- 在 `workflow.yaml.tool_scripts` 中声明 path 和 functions。
...
7. 生成 `workflow.yaml` skeleton。
8. 生成 `state.yml`。
9. 生成 `scenario.md`；仅为确实缺少实现的新工具生成脚本。
10. 执行结构、语义、工具、脚本和覆盖校验。
11. 将结果写入 Workflow draft，不自动发布。
...
只根据当前 Workflow draft、校验诊断和用户 repair hint 修复。

适用于：

- 手工创建的 Workflow。
- 用户已经大幅修改、无需再服从原 Skill 的 Workflow。
- UI 布局、YAML 语法、slot 引用等局部问题。

#### `source_aware`

除当前 Workflow draft 外，还读取固定的 `source_analysis_id/source_skill_revision_id` 和相关 evidence。

适用于：

- 从 Skill 生成的 Workflow。
- 修复遗漏步骤、错误工具映射、脚本转换或与来源规范不一致的问题。

`source_aware` 默认使用生成时固定的 Skill revision，不自动读取 Skill 最新 head。若用户要求基于最新 Skill，应走“重新分析/更新 Workflow”，不是普通 repair。
...
- 先诊断并生成 repair plan。
- 按 `workflow.yaml → state.yml → scenario.md/scripts → cross validation` 顺序执行。
...
`POST /workflow-drafts/{draft_id}:ai-generate`
...
- `GET /workflow-drafts/{draft_id}/generation-analysis`
- `POST /workflow-drafts/{draft_id}:confirm-workflow`
- `POST /workflow-drafts/{draft_id}:reanalyze-source`
...
扩展 `POST /workflow-drafts/{draft_id}:ai-repair`：
...
- `GET /workflow-drafts/{draft_id}/repair-runs/{repair_id}`
- 可选的 `POST /workflow-drafts/{draft_id}:repair-preview`，只生成诊断和 repair plan，不写 draft。
...
- `source_skill_revision_id` 与 Workflow `base_revision_id` 分别正确维护。
...
- workflow-local repair 不读取来源 Skill。
...
- description 驱动的普通 AI Workflow 生成保持可用。
- 现有 Workflow 发布、版本列表、rollback 和 runtime revision pinning 保持可用。
- 生成/repair 后的 Workflow 可由现有 PluginSpec、编辑器和 runtime 加载。
...
- 确定性诊断。
- workflow-local/source-aware 模式。
...
7. 生成结果写入 Workflow draft，发布后进入现有不可变 revision 体系。
8. Repair 支持 workflow-local 和 source-aware，并使用 optimistic lock 防止覆盖并发编辑。
