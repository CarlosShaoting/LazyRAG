...
> 对应 plan §3.1。`enable_workflow=false` 时根本不进入插件流水线，此处只处理已进入流水线的情况。
...
**Go `advanceAutoMode` — 全局 max_retries（非 workflow.yaml）**
...
def update_intent(scope: str, content: str, step_id: str | None = None) -> str:
    """UPSERT intent/constraint. scope: 'session' | 'step'. Workflow-agnostic."""
    ...
    if scope == 'session':
        ...
        # INSERT INTO workflow_step_intents ... ON CONFLICT (session_id, step_id) DO UPDATE
        TaskQueryDB().upsert_step_intent(session_id, step_id, content)
    return '约束已更新'
```

插件 `workflow.yaml` **无需**声明 `intent` 字段；约束完全由框架管理。
...
def advance_step_and_hand_off(
    ...
    """Advance a workflow step and END the current conversation turn immediately.
...
def advance_step(
    ...
    """Advance a workflow step and WAIT for its completion within this turn.
...
**不复用 `asyncjob.Enqueue("workflow_step_run", ...)`**
...
def create_schedule(cron_expr: str, prompt_template: str, timezone: str = 'Asia/Shanghai') -> str:
    """Create user schedule. NOT from workflow.yaml."""
...
```python
def list_workflow_steps(session_id: str | None = None) -> str:
