...
1. **Workflow 三级模式**：插件驱动模式分为三档，`default=dynamic`：
...
5. **缺失意图问询**：`dynamic` ChatAgent 可使用 `ask_user`；`auto` 模式在工具层禁用
   `ask_user`。auto 预检缺失信息时阻塞启动，等待用户下一轮主动补充（详见 plan §8.4）。
...
11. **修改次数限制**：仅在 auto 模式下生效；框架记录每个步骤的 DriverAgent RETRY 次数，超出**全局**上限（默认 3，见 plan §11.3，**非** `workflow.yaml` 配置）后强制 PASS，避免无限循环。dynamic 模式下不限制。

## 五、并行执行

12. **无依赖步骤并行**：当同一插件中存在多个互不依赖的步骤时（根据 state.yml 的 inputs 声明判断），支持同时启动多个 SubAgent 并行执行；所有并行步骤完成后再进入下一个依赖它们的步骤。
13. **并行状态感知**：前端 Workflow Panel 和 StateGraph 能正确展示多个步骤同时处于 running 状态的情况；DriverAgent 在并行步骤全部完成后才做整体裁决。

## 六、异步任务与定时触发

14. **异步 Job 支持**：插件步骤支持以异步 Job 形式提交执行，用户不必保持连接等待；Job 完成后通过 Conversation Events SSE 或通知机制告知用户结果。
15. **定时触发**：用户在对话中通过 ChatAgent 创建**用户级**定时规则（`user_schedules`，非 `workflow.yaml`）；每次到期在 `task_center_tasks` 产生一次执行实例（`schedule_id` 关联规则），与普通 Task 同列表展示。
