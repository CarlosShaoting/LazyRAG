...
> 在已落地的 Workflow `auto`/`dynamic`/`never` 驱动模式基础上，扩展插件驱动模式的精细化控制，补全 DriverAgent 能力，增加步骤级执行控制（停止 / 继续 / 重试 / 范围重跑），支持无依赖步骤并行执行，并引入 **TaskCenter**（对话级异步任务与定时触发）。
>
> 前置依赖：SubAgent 基础设施（`01-subagents`）、Workflow 机制（`02-workflow`）、数据历史与富媒体（`03-data_history`）**必须已落地**。
>
> 配套文档（渐进式披露，按需展开）：
...
| 字段 | 类型 | 默认 | 含义 |
| --- | --- | --- | --- |
| `enable_workflow` | bool | `true` | 是否启用插件机制；`false` 即纯问答（取代旧 `plugin_mode=never`） |
| `plugin_mode` | enum | `dynamic` | 仅 `enable_workflow=true` 时生效；`auto` 启用 DriverAgent 自动裁决推进，`dynamic` 禁用 DriverAgent、由 ChatAgent 自行决策 |
| `enable_subagent` | bool | `true` | 是否允许 ChatAgent 自主创建非 Workflow 的发散型 SubAgent；不影响插件步骤的 SubAgent |

`plugin_mode` 两档行为（仅在 `enable_workflow=true` 下）：
...
SubAgent 开关（`enable_subagent`）与插件步骤无关，控制 ChatAgent 是否允许自主发散创建非 Workflow 的 SubAgent，独立 bool 字段，默认 `true`：
...
```
用户全局默认（设置页）
  enable_workflow / plugin_mode / enable_subagent
       ↓ 进入对话时加载为初始值
...
| 层级 | 存储 | 说明 |
| --- | --- | --- |
| 用户全局默认 | `user_chat_settings`（新增表） | `user_id` + `enable_workflow` + `plugin_mode` + `enable_subagent` |
| 对话级覆盖 | `conversations.enable_workflow` / `conversations.plugin_mode` / `conversations.enable_subagent`（新增列，可空） | 空则回落全局默认；对话 UI 修改即写库 |
...
并行执行、停止中断、DriverAgent 裁决、定时触发，均由 Go 侧 `eventloop.go` / TaskCenter 控制。Python 的 `_trigger_workflow_step` / `advance_step` 只负责发射信号；多路并行由 Go 并发启动多个 `go subagent.Run(...)`。
...
ALTER TABLE conversations
  ADD COLUMN enable_workflow   BOOLEAN,       -- NULL → 回落 user_chat_settings
...
1. 读 `conversations.enable_workflow` / `plugin_mode` / `enable_subagent`；非空则用对话值。
2. 否则读 `user_chat_settings`；无记录则 `enable_workflow=true`、`plugin_mode=dynamic`、`enable_subagent=true`。
...
- **已有对话**：配置来自 `GET /conversations/{id}:detail` 返回的 `enable_workflow` 等字段（conversation detail 回填），**不**再调用 `getChatSettings`；用户在弹窗修改后直接调用 `PATCH /conversations/{id}/workflow-settings` 更新 DB，后续轮次生效。
- 前端**不需要**独立的 `GET /conversations/{id}/workflow-settings` 接口；后续消息 body 无需携带配置字段（Go 始终从 DB 读）。
...
步骤 SubAgent 完成时，先看 `enable_workflow`，再按 `plugin_mode` 决策：

- `enable_workflow=false` → 忽略插件事件（纯问答，根本不会进入插件流水线）
- `enable_workflow=true` 且 `plugin_mode=auto` → `go advanceAutoMode(...)` → `callDriverAgent` → 按 verdict 决策；`triggerNextChatTurn` 完成后若 Session 仍是 `active`，`checkAndFallbackIfStuck` 降级为 `waiting`
- `enable_workflow=true` 且 `plugin_mode=dynamic` → 发 `step_waiting`（`reason: dynamic_pause`），等用户下轮消息或按钮模拟消息
...
**接口**：`POST /api/core/conversations/{id}:stop`（扩展现有 `stopChatGeneration`，新增 workflow 语义分支）。
...
全局约束与步骤约束是**平台机制**，插件 `workflow.yaml` / `scenario.md` **不需要**声明字段，也**不需要**插件作者实现工具。
...
| `intent_context` | `workflow_step_intents`（新表） | `(session_id, step_id)` UNIQUE | 步骤级约束，如「第 2 步仅竖版」；无论该步骤执行多少次 attempt，约束只有一条，`update_intent` 直接 UPSERT |

```sql
CREATE TABLE workflow_step_intents (
...
- 每轮将 DB 中已有的全局 `intent_context`（`plugin_sessions`）与步骤级 `intent_context`（`workflow_step_intents`）序列化注入 user turn 或 system 附录，供 ChatAgent 在推理和调用 `advance_step` 时参考。

**注入 SubAgent**（SubAgent 启动时）：

- `resolve_plugin_injection` 在构建 SubAgent system prompt 时，同样注入当前 `plugin_sessions.intent_context`（全局约束）和对应 `workflow_step_intents` 记录（步骤级约束）。
...
> 状态查询工具（`list_workflow_steps` 等）见 §十三。
...
| **SubAgent / Workflow Step** | Task 的**子过程**，不入 Task 队列；仅在 Task 详情中关联查询 |
...
| 来源 | 是否创建 Task | 说明 |
| --- | --- | --- |
| 用户启用插件并触发 `trigger_*` / 首步 `advance_step` | **是**（`type=workflow_run`） | 默认创建；可在用户设置关闭「自动加入任务中心」 |
| 用户明确说「以后台任务方式执行」或点击「后台运行」 | **是**（`type=background_chat`） | 即使无插件 |
| 定时任务某次触发 | **是**（`type=scheduled`，`schedule_id` 指向定义） | 每次 cron 到期 **INSERT 新行**；与 `workflow_run` 等同表展示 |
| 普通纯问答 | 否 | 除非用户手动「加入任务中心」 |

Task 创建时机：Go 在首次 `task_created`（workflow_step 或 subagent）或用户显式 API 时 `INSERT task_center_tasks`。
...
task_center_tasks（单次执行实例，task_type=scheduled，schedule_id=定义.id）
       │ 与普通 workflow_run / background_chat 并列展示
       ▼
标准 chat/workflow 管道（conversation_id 可复用绑定会话或新建）
```
...
| SubAgent / Workflow Step | `sub_agent_tasks` / `plugin_session_steps` | 否；在 Task 详情中关联查询 |
...
CREATE TABLE task_center_tasks (
  ...
  task_type         VARCHAR(32)  NOT NULL,  -- workflow_run | background_chat | scheduled
  title             TEXT,
...
Worker：Task 进入 `running` 后，本质仍是标准 chat/workflow 管道；TaskCenter 仅**跟踪**状态，不替代 `subagent.Run`。

---

## 十、定时任务（用户行为，与 workflow 无关） 

### 10.1 原则

- **不**在 `workflow.yaml` 配置 cron。
...
**注入原则**：这三个工具与 `enable_workflow` 无关，在 `chat_service.py` 中**无条件注入**（独立于 `resolve_plugin_injection`），即使 `enable_workflow=false` 也可用。
...
**非** `workflow.yaml` 配置。全局默认 `max_retries=3`（`backend/core/workflow/config.go` 或环境变量 `LAZYMIND_DRIVER_MAX_RETRIES`，**不**暴露给插件作者）。
...
PATCH /api/core/user/chat-settings
  body: { "enable_workflow", "plugin_mode", "enable_subagent" }

PATCH /api/core/conversations/{id}/workflow-settings
  body: { "enable_workflow", "plugin_mode", "enable_subagent" }   # 对话级覆盖
...
POST /api/core/conversations:chat
  body: enable_workflow, plugin_mode, enable_subagent（可选，对话级）
...
| 工具 | 作用 |
| --- | --- |
| `list_workflow_steps` | 列出各步骤状态（只读） |
...
1. **配置与数据层**
   ...
   - `plugin_sessions.intent_context` JSONB；`workflow_step_intents` 新表（`(session_id, step_id)` UNIQUE）；`plugin_sessions.parallel_step_ids` JSONB。
   - `task_center_tasks`、`user_schedules` 表。

2. **Go**
   - 配置加载与透传；`eventloop.go` 按 `enable_workflow` + `plugin_mode` 分叉。
   ...
   - TaskCenter repository + scheduler；**不**注册 `workflow_step_run` 到 asyncjob。

3. **Python**
   - `advance_step`（同步，FileSystemQueue 轮询等待）+ `advance_step_and_hand_off`（结束当前轮次，加入 `plugin_stop_tools`）。
   - 按 `plugin_mode` 动态注册工具（§3.3）。
   - `driver_agent.py`：llm_config、附件 + **workflow artifact** 读取。
   - `plugin_manager.py`：`update_intent`、`ask_user`、查询工具；框架内置意图管理提示词。
   - 工具注入规则拆分：`resolve_plugin_injection` 只负责插件相关工具和提示词；`_build_chat_agent_task_context` 移到 `chat_service.py`，条件改为 `enable_workflow or enable_subagent`（非仅 `enable_workflow`）；`create_schedule` / `list_schedules` / `cancel_schedule` 在 `chat_service.py` 无条件注入，不经 `resolve_plugin_injection`。
...
4. **前端**
   - 设置页 + 对话级 `enable_workflow` / `plugin_mode` / `enable_subagent` UI。
