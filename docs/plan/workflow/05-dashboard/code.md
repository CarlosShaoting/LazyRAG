...
**`backend/core/workflow/handlers.go`**
...
func (h *Handler) GetStateGraph(c *gin.Context) {
    ...
    // 2. 调 Python 取 workflow spec（含完整 state.yml 结构）
    spec, err := h.pythonClient.GetPluginSpec(ctx, session.PluginID)
    if err != nil {
        c.JSON(http.StatusInternalServerError, gin.H{'error': 'failed to load workflow spec'})
        return
    }
...
```go
pluginGroup.GET('/workflow-sessions/:session_id/state-graph', pluginHandler.GetStateGraph)
```
