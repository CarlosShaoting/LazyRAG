...
- PluginPanel Tab 由 `workflow.yaml` 的 `ui.tabs` 声明驱动，StateGraph 是框架级系统能力，不属于插件内容
...
### 2.1 新增接口：`GET /workflow-sessions/{session_id}/state-graph`
...
| `nodes[].label` | 展示名称，优先用 workflow.yaml `steps[].label`，无则用 `id` |
...
**Go 实现逻辑**（`backend/core/workflow/handlers.go` 新增 `GetStateGraph`）：

1. 读 `plugin_sessions` 取 `plugin_id`、`current_step_id`、`status`
2. HTTP 调 Python `GET /api/workflows/{plugin_id}` 取完整 `state` 字段（已含 `transitions` + `steps` + `initial`）
...
```
GET /workflow-sessions/{session_id}/state-graph → workflow.GetStateGraph
```
...
`liveRefresh=true` 时，`StateGraphModal` 从 `useTaskCenterStore` 订阅 `convEventBus`：当收到 `step_waiting` / `step_partial_done` / `plugin_completed` / `plugin_error` 事件时，重新调用 `GET /workflow-sessions/{session_id}/state-graph` 更新 `nodes` 状态。布局（dagre 计算结果）缓存，仅替换节点 `status` 字段，不重新布局。
...
- 仅 `task.plugin_session_id` 存在（即 `task_type=workflow_run`）时才可点击
...
1. `backend/core/workflow/handlers.go`：新增 `GetStateGraph` handler（约 80 行）
2. `backend/core/routes.go`：注册路由 `GET /workflow-sessions/{session_id}/state-graph`
...
- [ ] `GET /workflow-sessions/{session_id}/state-graph` 返回正确的 nodes/edges/current_step_id
...
- [ ] TaskCenter 步骤列点击能打开 Modal（workflow_run 类型任务）
