...
退出：非白名单代码和公共 schema 不含旧 Workflow 领域名；旧数据库 fixture 可完整读写；旧 binary 在 expand 后 schema 上通过核心读写测试；新 reader 可读取未 backfill 旧行；feature flag 可切回旧路由别名。不得改变状态机语义。
...
必须交付：`plugin_session_steps` 的兼容 lease/heartbeat/fencing 扩展、queued Attempt、通用 `workflow_outbox`、progress/complete/fail/cancel 和 FakeExecutor。旧 `workflow_run_outbox` 继续服务旧 worker，新旧 claim 域必须隔离。
...
必须交付：schema/service capability 均满足后默认启用 queued Attempt，关闭 Core 直接 `/api/subagent/run` 调用；旧 endpoint 和 `workflow_run_outbox` 仅作兼容入口。
