...
`docs/plan/workflow/6-create/image.png`
...
```
src/modules/workflow/components/StateGraphEditor/
...
| 交付物 | 说明 |
|--------|------|
| `StateGraphEditor` 前端组件 | 可独立嵌入 Workflow Detail 页 |
| `core/validator.ts` + 单元测试 | 10 条校验规则 |
| `core/parser.ts` + `core/serializer.ts` + 单元测试 | YAML ↔ GraphModel 双向转换 |
| 后端 Draft 保存接口（校验兜底）| `POST /workflows/:id/draft`，接收 YAML，Go 侧同一套规则二次校验后存库 |
...
- 权限控制（管理员限定）→ 计划 2
- Skill → 插件草稿生成 → 计划 2
- scenario 完整展示（workflow.yaml / scenario.md / driver.md）→ 计划 2
