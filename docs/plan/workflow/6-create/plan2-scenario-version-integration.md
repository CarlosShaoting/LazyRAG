...
- Scenario 多文件完整展示（`workflow.yaml`、`scenario.md`、`driver.md`、`scripts/`、`state.yml`）
...
| 文件 | 说明 |
|------|------|
| `workflow.yaml` | 插件元数据、UI slot 声明、trigger 配置 |
...
| 接口 | 方法 | 说明 |
|------|------|------|
| `GET /workflows/:id/files` | 返回文件树结构 | 列出所有文件路径 |
| `GET /workflows/:id/files/*path` | 返回单文件内容 | 支持 `?version=draft|v1|v2` |
| `PUT /workflows/:id/files/*path` | 更新单文件内容（仅 Draft）| 非 `state.yml` 的其他文件编辑入口 |
...
| 接口 | 方法 | 说明 |
|------|------|------|
| `GET /workflows/:id/stategraph` | 读取状态机 | `?version=draft|v1|v2|latest`；返回 YAML 文本 + 布局 + 图结构摘要 |
| `POST /workflows/:id/draft` | 保存 Draft | 接收 YAML 文本，执行校验，写入 `plugin_versions` 表（draft 记录）|
| `POST /workflows/:id/publish` | 发布 | 将当前 Draft 版本号自增发布为正式版本，触发热重载 |
...
| 接口 | 说明 |
|------|------|
| `GET /workflows/:id/versions` | 返回所有已发布版本列表（含版本号、发布时间、发布者、备注）|
| `POST /workflows/:id/versions/:version/rollback` | 基于历史版本内容创建新版本号并发布；版本历史不可变 |

### 4.4 版本差异对比（条目 22）

- 后端接口：`GET /workflows/:id/diff?from=v1&to=v2`，返回节点/边的增删改列表
...
目前 scenario/workflow 由文件系统目录在服务启动时静态加载。动态创建后需要让 Python chat 服务在运行期感知到新插件/新版本。
...
发布接口（Go core）
  └─► 写入 plugin_versions 表（status=published）
        └─► 发布 Redis channel "workflow.published" 事件
...
用户发起新建 Chat（POST /conversations，带 plugin_id）
  └─► Go core 查询该 workflow 的 latest published version_id
...
```
SKILL.md 内容
  └─► Go core 调用 Python AI 分析服务（POST /ai/generate-workflow-draft）
        └─► LLM 分析工作流描述，生成：
              ├─ scenario.md（意图识别描述）
              ├─ state.yml（步骤列表 + 状态机转移）
              └─ workflow.yaml（插件元数据 + UI slot 声明）
...
| 接口 | 方法 | 权限 | 说明 |
|------|------|------|------|
| `GET /workflows/:id/files` | GET | 所有登录用户 | 文件树 |
| `GET /workflows/:id/files/*path` | GET | 所有登录用户 | 文件内容，支持 version 参数 |
| `PUT /workflows/:id/files/*path` | PUT | 管理员 | 更新非 state.yml 文件 |
| `GET /workflows/:id/stategraph` | GET | 所有登录用户 | 读取状态机（含布局）|
| `POST /workflows/:id/draft` | POST | 管理员 | 保存 Draft（含校验）|
| `POST /workflows/:id/publish` | POST | 管理员 | 发布新版本，触发热重载 |
| `GET /workflows/:id/versions` | GET | 所有登录用户 | 版本列表 |
| `POST /workflows/:id/versions/:v/rollback` | POST | 管理员 | 回滚到历史版本 |
| `GET /workflows/:id/diff` | GET | 管理员 | 版本差异对比 |
| `POST /workflows/generate-from-skill` | POST | 管理员 | Skill → 草稿生成 |
| `POST /workflows` | POST | 管理员 | 从零新建空插件 |
...
| `workflows` | 新增字段 `latest_version_id` | 指向最新发布版本 |
