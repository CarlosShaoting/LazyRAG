...
> 基于已落地的 SubAgent 基础设施，为 ChatAgent 设计 Workflow 执行机制：每个插件由多个 Step 组成，每个 Step 是一个异步 SubAgent；ChatAgent 负责意图识别与步骤触发；前端复用 Task Center 面板展示各步骤执行过程。
>
> 配套文档：
> - 示例 / 伪代码：[`code.md`](./code.md)
> - 端到端交互路径（文生图场景）：[`trace.md`](./trace.md)
>
> 前置依赖：SubAgent 基础设施（`docs/plan/workflow/01-subagents/plan.md`）**必须已落地**。
...
### 1.1 Workflow = 多 Step 有序 SubAgent 序列

插件（Workflow）是一个**有状态的多步骤工作流**，每个 Step 是一个独立的异步 SubAgent：

- **插件**：声明式描述（`workflow.yaml` + `scenario.md` + `state.yml`），定义步骤集合、状态机转移、依赖关系。
- **Step**：等同于一个 SubAgent，有自己的 `sub_agent_tasks` 记录、`sub_agent_steps` 执行轨迹、`sub_agent_artifacts` 成果，走完整 SubAgent 生命周期。
- **插件会话**（Workflow Session）：一次用户触发产生一个会话，持有当前进行中的步骤状态与全部已产出 artifact。

核心简化：**Workflow 机制不引入新的 Python 执行引擎**，完全复用 SubAgent 的 `/api/subagent/run` 端点、`SubAgentDB`、事件流协议。Workflow 层只是在 ChatAgent 工具层 + Go 编排层增加了「步骤序列化」和「跨步依赖注入」逻辑。

### 1.2 auto 含义重新定义

原 SubAgent plan 中 `auto/manual` 指 ChatAgent 是否同步等待 SubAgent 完成。在我们的新Workflow 方案中：

- **所有ChatAgent自主调用的非Workflow的SubAgent均同步执行**（等同于原 auto 模式）。
- **所有Workflow Step（SubAgent）均异步执行**（等同于原 manual 模式）。
...
ChatAgent 的 `create_subagent` 调用中 **`auto` 参数不再出现**——Workflow Step 统一走异步，mode 由 Go 侧提供的全局配置来决定。
...
每个 Step 对应一个 `sub_agent_tasks` 记录，前端在 Task Center 中分组（按 Workflow Session）展示各步骤进度与产物。

### 1.4 设计原则：ChatAgent 不调用 StepAgent

ChatAgent **不直接调用 Step 执行端点**（`/api/subagent/run`）。LLM 调用的是具有业务语义的 Workflow 工具（`trigger_<plugin_id>` / `advance_step`），工具内部经由 `_trigger_workflow_step()` 最终调用 `create_subagent(agent_type='workflow_step', ...)` 向 Go 发出 `task_created` 信号。Go 识别到这是一个 Workflow Step 后，按 SubAgent 协议调 `/api/subagent/run`，SubAgent 框架执行具体逻辑。

这一封装层（见 [§4.1](#41-chatagent-workflow-工具)）使 LLM 只需填写业务参数（`user_input`、`step_id`），无需感知底层的 `agent_type`、`session_id` 等字段，同时保持 Go 作为唯一 Orchestrator 的架构约束不变。
...
```
前端 ←──SSE──→ Go（Workflow EventLoop） ←── ChatAgent（意图识别 / 步骤决策）
                                       ←── StepAgent = SubAgent（步骤执行）
```

- Go 持有 Workflow Session 状态，驱动步骤序列。
...
- **Go 层（硬约束）**：收到 `task_created`（`agent_type='workflow_step'`, `is_cold_start=true`）时，查询该 conversation 是否已存在 `status='active'` 的 `plugin_sessions` 记录；若存在则拒绝创建新 session，向主 SSE 返回 error 事件。

---

## 二、数据层（新增 3 张表，复用 3 张 SubAgent 表）

SubAgent 的三张表（`sub_agent_tasks` / `sub_agent_steps` / `sub_agent_artifacts`）**不修改**，Workflow Step 的执行数据完整存入其中（`agent_type = 'workflow_step'`）。
...
> `plugin_session_steps.status` 是 `sub_agent_tasks.status` 的镜像，由 Go 在处理 SubAgent 事件时同步更新，方便 Workflow 层查询而无需 JOIN。

**`sub_agent_tasks` 在 Workflow 场景的字段语义**：

| 字段 | Workflow 场景取值 |
| --- | --- |
| `agent_type` | `'workflow_step'` |
| `title` | `'{plugin_id}:{step_id}'`（如 `'image-workflow:optimize_prompt'`） |
...
记录每个 Slot 的写入历史，是 Workflow Panel 的数据基础：

```sql
CREATE TABLE plugin_slot_revisions (
    id              VARCHAR(36)  PRIMARY KEY,
    session_id      VARCHAR(36)  NOT NULL REFERENCES plugin_sessions(id),
    slot_id         VARCHAR(64)  NOT NULL,    -- 对应 workflow.yaml ui.tabs[].slots[].id
...
每个插件的所有文件统一放在 `workflow/workflows/<workflow-id>/` 下：

```
workflow/workflows/<workflow-id>/
  workflow.yaml              # 注册元数据（必须）
...
Workflow 机制在 ChatAgent 工具层做最小扩展，**不引入新的工具调用协议**。

### 4.1 ChatAgent Workflow 工具
...
`trigger_<plugin_id>` 和 `advance_step` 内部均调用 `_trigger_workflow_step()`，执行两层校验：

1. **格式校验**：step_id 在状态机可达步骤中；user_input 非空。
2. **依赖状态校验**：从 DB 查依赖 artifact 的产出状态，按 required/optional 语义判断。

校验通过后，写入 `task_created` 信号（`agent_type='workflow_step'`），Go 识别并创建 Workflow Session Step。

### 4.2 `task_created` 信号（Workflow Step 专用字段）

Workflow Step 的 `task_created` 沿用 SubAgent 协议（见 SubAgent plan 3.1），`agent_type='workflow_step'`，`params` 中额外携带：
...
**插件自定义工具**：`workflow.yaml` 的 `tool_scripts` + state.yml step 的 `tools` 字段声明的函数（如 `dalle_generate`、`enhance_image_tool`）。
...
- Workflow Step 的提示词由 Go 将 `state.yml step.prompt` + 注入的 artifacts 拼好后通过 `objective` 字段下发，框架照常执行。

新增 Python 组件仅在 ChatAgent 工具层：

- `workflow/workflows/` 目录结构 + `workflow.yaml` 解析（`plugin_loader.py`）。
- `trigger_<plugin_id>` 和 `advance_step` 工具注册（`plugin_manager.py`）。
- 依赖状态校验（`_trigger_workflow_step`）。

### 5.2 Go 层：Workflow EventLoop

Go 处理 `task_created`（`agent_type='workflow_step'`）时走 Workflow 专属路径：
...
当 SubAgent 产出 artifact 事件时，Go 调用 Python 的 `GET /api/workflow/slot-binding?plugin_id=...&slot=...` 查询该 slot 对应的 `slot_id` 和 `cardinality`：

```
artifact 事件到达
  → GET /api/workflow/slot-binding?plugin_id=<pid>&slot=<slot>
      → Python 查 state.yml outputs[].slot + workflow.yaml ui.slots[].cardinality
...
SubAgent done
  ...
  → auto：
      ...
      2. 调 DriverAgent（POST /api/workflow/driver）→ 解析 <verdict>
...
### 5.3 Go Workflow Session 状态机

Go 维护 Workflow Session 的 step 状态，用于依赖校验兜底：
...
ChatAgent 的 `_trigger_workflow_step` 已做主路径依赖校验，Go 侧的检查作为防御性断言。
...
Go 通过 HTTP POST `/api/workflow/driver` 调用 Python DriverAgent。
...
### 6.1 Workflow Session API

```
GET  /api/core/conversations/{conv_id}/workflow-sessions
     → plugin_sessions WHERE conversation_id=? ORDER BY created_at DESC
     → 含各 session 的 current_step_id 和 status

GET  /api/core/workflow-sessions/{session_id}
     → session 详情 + plugin_session_steps 列表（含 task_id 用于关联 artifacts）

GET  /api/core/workflow-sessions/{session_id}/slots
...
GET /api/core/workflows/{plugin_id}
...
Workflow Panel 是对话视图中的一个独立区块，**始终挂载在当前会话 bot 最后一条消息之后**。一个会话只有一个活跃 Panel，对应唯一一个 `plugin_session`。
...
```
页面加载 / 会话切换
  → GET /conversations/{id}/workflow-sessions:latest   （获取当前活跃 session）
  → GET /workflow-sessions/{session_id}/slots           （初始化全量 Slot 内容）
  → GET /workflows/{plugin_id}                          （获取 ui.tabs 结构用于渲染）

运行时（Step 执行中）
  → PluginPanel 以 3 秒间隔轮询 GET /workflow-sessions/{session_id}/slots
...
Workflow Panel 支持两种渲染模式，框架在加载插件时按优先级选择：

**模式一：插件自定义渲染（优先）**

插件提供 `frontend/index.tsx`，直接编排框架提供的基础组件（`SlotImage` / `SlotText` / `SlotFile` 等），自主决定布局、Tab 组织方式和交互细节。`workflow.yaml` 的 `ui` 声明在此模式下仍需存在，用于 Go 层写入 Slot Revision，但前端布局完全由 `index.tsx` 控制。

**模式二：框架自动渲染（降级）**

插件不提供 `frontend/index.tsx` 时，框架读取 `workflow.yaml` 的 `ui` 声明，按标准模板自动生成 Panel：TabBar + 每个 Tab 内按 Slot 顺序平铺内容区，无需插件编写任何前端代码。

```
frontend/index.tsx 存在？
  ├── 是 → 插件自定义渲染（完全控制布局）
  └── 否 → 框架按 workflow.yaml ui 声明自动渲染（标准 Tab + Slot 模板）
```
...
### 7.4 `workflow.yaml` ui 声明的前端消费

`ui` 声明在两种模式下各有用途：

- **自动渲染**：前端通过 `GET /api/core/workflows/{plugin_id}` 获取 `ui` 声明，直接按 tabs / slots / cardinality 生成 Panel 骨架。
- **自定义渲染**：`ui` 声明由 Go 层消费（写 `plugin_slot_revisions` 时判断 cardinality），前端 `index.tsx` 通过 `useSlot(slotId)` Hook 订阅数据，不直接解析 `ui` yaml。

两种模式下，Slot 初始内容均来自 `GET /workflow-sessions/{id}/slots`，运行时增量更新均通过 Task SSE 的 `artifact` 事件完成。

---

## 八、实施顺序

Workflow 机制以 SubAgent 基础设施为前提，不重复其实施步骤。

1. **插件文件结构 + Loader**：
   - `workflow/workflows/` 目录约定。
   - `plugin_loader.py`：解析 `workflow.yaml` / `state.yml` / `scenario.md` / `driver.md`，startup 校验（`driver.md` 缺失时 auto step → error）。
   - `state.yml` StateMachine（`is_reachable` / `get_reachable_steps`）。

2. **ChatAgent Workflow 工具层**：
   - `plugin_manager.py`：`build_cold_start_tools()`（per-workflow `trigger_<id>`）和 `build_advance_tool(session)`。
   - `_trigger_workflow_step()`：两层校验 + 写 `task_created`（`agent_type='workflow_step'`）。
...
3. **数据层**：
   ...
   - Go Workflow Session CRUD（`CreateSession` / `UpdateSessionStep` / `WriteSlotRevision`）。

4. **Go Workflow EventLoop**：
   - `task_created`（`agent_type='workflow_step'`）走 Workflow 路径：冷启动时检查 active session 唯一性约束，分配 session + step 记录，构造 objective，调 `createSubAgentTask`，`go runSubAgent`。
   - `routeToTaskSSE` 的 `artifact` 分支：检查 `slot_id` 绑定，写 `plugin_slot_revisions`，维护 `selected` 字段。
   - `routeToTaskSSE` 的 `done` 分支新增 Workflow 推进逻辑（auto → DriverAgent → ChatAgent；manual → step_waiting）。
...
6. **Workflow Session API**：`/workflow-sessions`、`/workflow-sessions/{id}`、`/workflow-sessions/{id}/slots` 三个端点；`/workflows` 接口在 `ui` 字段中返回完整 Tab/Slot 声明。

7. **前端 Workflow Panel**：
   - 框架提供基础 Slot 组件（`SlotImage` / `SlotText` / `SlotFile`）和 `useSlot` Hook。
   - `PluginPanel` 容器：优先加载插件的 `frontend/index.tsx`；不存在时按 `workflow.yaml` ui 声明自动渲染标准 Tab/Slot 模板。
   - 初始化时调 `/workflow-sessions/{id}/slots` 填充已有内容。
   - 复用 Task SSE 订阅，监听 `artifact` 事件，按 `slot` 映射实时更新 Slot。
   - `step_waiting` 事件触发「等待用户确认」UI 状态。
   - `plugin_completed` 事件触发插件完成通知。

8. **首个插件（image-workflow）端到端验证**：
   - `workflow/workflows/image-workflow/` 完整文件（含 `ui` 声明和 slot 绑定）。
