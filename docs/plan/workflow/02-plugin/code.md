...
- [C1. scenario.md 示例（image-workflow）](#c1)
- [C2. `_trigger_workflow_step` 工具实现（两层校验 + task_created）](#c2)
- [C3. `chat_service.py` Workflow 工具注入](#c3)
- [C4. Go Workflow EventLoop（task_created 分支 + done 推进）](#c4)
...
## C1. scenario.md 示例（image-workflow）
...
## C2. `_trigger_workflow_step` 工具实现（两层校验 + task_created）
...
from lazymind.chat.workflow import plugin_loader
...
def _trigger_workflow_step(
    ...
    if sm is None:
        return f'Error: workflow {plugin_id!r} not found.'
    ...
    if inputs and not is_cold_start and session_id:
        ...
        try:
            resp = httpx.get(f'{core_url}/workflow-sessions/{session_id}', timeout=3.0)
    ...
    _write_agent_data(
        ...
        mode='manual',          # Workflow step 统一异步；Go 控制是否自动推进
...
## C3. `chat_service.py` Workflow 工具注入
...
async def handle_chat(query, history, mode, plugin_context=None, **kwargs):
    ...
    # resolve_plugin_injection 封装了所有 workflow-context 分支逻辑：
...
## C4. Go Workflow EventLoop（task_created 分支 + done 推进）

> 拦截点是现有 SubAgent upstream 消费循环（`d.TaskCreated != nil`），Workflow Step 通过 `agent_type='workflow_step'` 走专属分支。
...
// routeToTaskSSE done 分支新增 Workflow 推进逻辑
