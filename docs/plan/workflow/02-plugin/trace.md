# Workflow 方案 · 交互路径（文生图场景）

> 配合 [`plan.md`](./plan.md) 阅读，示例代码见 [`code.md`](./code.md)。本文件走通一个端到端场景，用于验证各组件协作是否自洽。

**场景**：用户发送「帮我画一只戴帽子的猫」，**auto 模式（mode: auto）**，插件为 `image-workflow`（五个 Step：`analyze_subject` → `collect_materials` → `optimize_prompt` → `generate_image` → `enhance_image`）。
...
_trigger_workflow_step() 执行：
  ...
  _write_agent_data('task_created',
    ...
    params={'plugin_id':'image-workflow','step_id':'analyze_subject',
...
translator 翻译后，主 SSE 发出:
  ...
  data: {"task_created":{"task_id":"task-001","title":"image-workflow:analyze_subject",...}}
  data: [DONE]
```

---

## T=2：Go 处理 task_created（Workflow Step 分支）

Go 在 upstream 消费循环识别 `d.TaskCreated != nil` 且 `agent_type='workflow_step'`：

```
Go → DB:   INSERT plugin_sessions
           {id:'ps-001', conversation_id:'conv-001',
            plugin_id:'image-workflow', trigger_history_id:'h-001',
...
Go → DB:   INSERT sub_agent_tasks
           {id:'task-001', conversation_id:'conv-001', trigger_history_id:'h-001',
            ...
            agent_type:'workflow_step', title:'image-workflow:analyze_subject',
...
FE:   Task Center 出现 "image-workflow" 分组，含 "analyze_subject" 任务卡片（pending）
...
artifact    → DB: INSERT sub_agent_artifacts {task_id:'task-001', artifact_key:'subject_analysis',...}
            ...
            → Redis RPUSH → FE（Task SSE）
            FE: Workflow Panel "Analysis" Tab 展示分析结果
...
```
Go → Python:  POST /api/workflow/driver
Body: {
...
_trigger_workflow_step('collect_materials', '收集戴帽子的猫的参考素材'):
  ...
  _write_agent_data('task_created',
    task_id='task-002',
    agent_type='workflow_step',
    params={'plugin_id':'image-workflow','step_id':'collect_materials',
...
主 SSE:
  data: {"task_created":{"task_id":"task-002","title":"image-workflow:collect_materials","plugin_session_id":"ps-001"}}
  data: [DONE]
```
...
SubAgent 执行（sid=task-002）:
  ...
  FE: Workflow Panel "Materials" Tab 实时追加参考图
...
Go 消费:
  artifact → DB: INSERT sub_agent_artifacts {artifact_key:'generated_image_url',...}
           ...
           → FE: Workflow Panel "Result" Tab 展示原始图
...
SubAgent task-005 执行 enhance_image，产出 enhanced_image_url artifact：
  FE: Workflow Panel "Result" Tab 新增增强图列表项（cardinality=list 追加）
...
Go:  parseVerdict → DONE
     ...
     → Conversation Events SSE: {"type":"plugin_completed","session_id":"ps-001","plugin_id":"image-workflow"}
     → 不再触发新一轮 ChatAgent，auto loop 结束

FE:  收到 plugin_completed 事件，停止 slots 轮询，刷新最终 Slot 内容
FE:  Task Center 中 image-workflow 分组所有步骤 ✓（共 5 个任务卡片）
```
...
Body: {
  query: "继续",
  plugin_context: {session_id:'ps-001', plugin_id:'image-workflow', current_step:'analyze_subject'}
}
...
```
FE → Go:  GET /api/core/conversations/conv-001/workflow-sessions
...
Go → FE:  {
  sessions: [{
    id: 'ps-001', plugin_id: 'image-workflow', status: 'completed',
    steps: [
...
Body: {
  query: "继续",
  plugin_context: {session_id:'ps-001', plugin_id:'image-workflow', current_step:'generate_image'}
}
