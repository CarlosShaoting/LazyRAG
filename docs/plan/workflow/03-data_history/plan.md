...
> 在已落地的 Workflow 执行管道基础上，补齐 artifact 的版本历史、富媒体输入输出、跨步骤上下文携带与前端展示增强能力。前置依赖：SubAgent 基础设施（`01-subagents/plan.md`）与 Workflow 机制（`02-workflow/plan.md`）**必须已落地**。
...
阶段 1/2 已建立的六张表（`sub_agent_tasks` / `sub_agent_steps` / `sub_agent_artifacts` / `plugin_sessions` / `plugin_session_steps` / `plugin_slot_revisions`）**结构不修改**。本阶段新增两张表（`plugin_slot_order`），并在 `sub_agent_artifacts` / `plugin_slot_revisions` / `workflow.yaml` 上扩展字段。
...
2. workflow.yaml slot 声明了静态 type（text / image / html / json）
     → 按静态 type 渲染
...
> `workflow.yaml` slot 的 `type` 字段仍为必填，用于声明该 slot 期望的内容类型；动态类型推断是对静态声明的**补充**，而不是替代。
...
```
DELETE /api/core/workflow-sessions/{session_id}/slots/{slot_id}/items/idx/{list_index}
...
```
PATCH /api/core/workflow-sessions/{session_id}/slots/{slot_id}/order
...
现有链路（`chat_service.py` 中 `files` 参数 → `validate_and_resolve_files`）已支持将文件路径注入 ChatAgent 上下文。本阶段扩展到 Workflow Step 场景：
...
- `focused_tab`：前端随 chat 请求携带，值为用户当前停留的 Tab id（如 `materials` / `slides`）；Workflow Panel 收起时为 `null`。
...
`workflow.yaml` 的 `summary_max_chars` 字段可覆盖 200 的默认截断长度。
...
```
POST /api/core/workflow-sessions/{session_id}/slots/{slot_id}/items/idx/{list_index}/rollback
  body: { "revision": 3 }
...
```
GET /api/core/workflow-sessions/{session_id}/slots/{slot_id}/items/idx/{list_index}/versions
...
- UI图参考docs/plan/workflow/03-data_history/image_version_ui.png
...
`workflow.yaml` 支持 `i18n` 字段覆盖展示文案：
...
`plugin_loader.py` 解析 `i18n` 字段，`GET /api/core/workflows/{plugin_id}` 接口新增 `accept-language` 响应字段，前端按当前语言选取对应 label。
...
PATCH  /api/core/workflow-sessions/{session_id}/slots/{slot_id}/items/idx/{list_index}
...
GET /api/core/workflow-sessions/{session_id}/slots/{slot_id}/order
...
1. ✅ **数据层**：
   ...
   - `workflow.yaml` schema 扩展（`ordered` / `layout` / `i18n`），`plugin_loader.py` 同步解析。
...
3. ✅ **Python 层**：
   ...
   - `workflow.yaml` i18n 解析，`GET /workflows/{id}` 接口返回多语言 label。
