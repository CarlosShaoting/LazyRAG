...
**最新产品规则变更（用户确认）：工具改为默认开放，模型仍默认关闭。** 工具授权无记录时按允许处理，有显式 enabled=false 时拒绝；所有可用性、归属、启用状态和执行上下文检查仍保留。新增可用工具自动遵循该默认值，不覆盖已有关闭设置。本文后续关于“全部能力默认关闭”的原始 PRD/验收记录属于变更前基线。
