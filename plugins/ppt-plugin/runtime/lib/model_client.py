...
# ---------------------------------------------------------------------------
# Injectable backends (LazyMind SubAgent uses AutoModel via these hooks)
# ---------------------------------------------------------------------------
...
def set_llm_impl(fn: Any) -> None:
    """Install a custom llm(system, user, **kw) -> str callable (or None to clear)."""
    global _LLM_IMPL
    _LLM_IMPL = fn


def set_vlm_impl(fn: Any) -> None:
    """Install a custom vlm(system, user, images, **kw) -> str callable (or None to clear)."""
    global _VLM_IMPL
    _VLM_IMPL = fn
...
def vlm(system_prompt: str, user_prompt: str, images: list[str | Path], *,
    ...
    if _VLM_IMPL is not None:
        return _VLM_IMPL(system_prompt, user_prompt, images, model=model)
