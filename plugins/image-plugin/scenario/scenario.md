...
1. **analyze_subject** — 用户可读的需求分析（`subject_analysis`）+ 内部路由（`workflow_routing`，不在分析 Tab 展示）
2. **collect_materials** — 联网搜图 **1–3 张**（REFERENCE_GENERATE）或准备编辑底图（FIND_AND_EDIT / EDIT_UPLOAD）
...
2. **analyze_subject**：需求分析给用户看（`subject_analysis`）；路由元数据写入 `workflow_routing`（不在分析 Tab 展示）。KB 检索由系统在触发前完成。
...
7. 编辑类请求（FIND_AND_EDIT / EDIT_UPLOAD）禁止 advance 到 `generate_image`。
8. KB_STYLE / CREATE_NEW：analyze_subject 完成后直接 advance 到 `optimize_prompt`，禁止进入 `collect_materials` 搜图。
9. REFERENCE_GENERATE：collect_materials 完成后 **必须** `advance_step(optimize_prompt)`，再进入 `generate_image`；**禁止** advance 到 `enhance_image`。
10. FIND_AND_EDIT / EDIT_UPLOAD：collect_materials 完成后 **必须** `advance_step(optimize_prompt)`，再进入 `enhance_image`；**禁止** advance 到 `generate_image`。
11. optimize_prompt 完成后：生成类 WORKFLOW → `generate_image`；编辑类 WORKFLOW（FIND_AND_EDIT / EDIT_UPLOAD）→ `enhance_image`。
12. EDIT_UPLOAD：analyze_subject 完成后 advance 到 `collect_materials`，保存上传原图，再 optimize → enhance。
