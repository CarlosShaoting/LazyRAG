...
func TestAddGroupModelDiscoversVLLMMaxModelLen(t *testing.T) {
	db := setupListProviderTestDB(t)
	store.Init(db, db, nil)
	t.Cleanup(func() { store.Init(nil, nil, nil) })

	const modelName = "Qwen/Qwen3.8-Flash-Next"
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/v1/models" {
			t.Fatalf("metadata path = %q, want /v1/models", r.URL.Path)
		}
		...
		_, _ = w.Write([]byte(`{
			...
			"data":[{
				...
				"created":1789032282,
				"owned_by":"vllm",
				"root":"/mnt/lustre/share_data/dengyuang/models/Qwen3.8-Flash-Next",
				"parent":null,
	...
	req := httptest.NewRequest(
		http.MethodPost,
		"/api/core/model_providers/"+provider.ID+"/groups/"+group.ID+"/models",
		strings.NewReader(`{"name":"`+modelName+`","model_type":"llm"}`),
	)
	req.Header.Set("X-User-Id", "user-1")
	req = mux.SetURLVars(req, map[string]string{
		"model_provider_id": provider.ID,
	...
	var stored orm.UserModelProviderGroupModel
	if err := db.Where("name = ?", modelName).Take(&stored).Error; err != nil {
		t.Fatal(err)
	}
	if stored.MaxInputTokens == nil || *stored.MaxInputTokens != "262144" {
		t.Fatalf("stored max_input_tokens = %v, want 262144", stored.MaxInputTokens)
	}
}
