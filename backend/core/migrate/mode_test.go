...
func TestRepositoryStructuredMigrationCatalogLoads(t *testing.T) {
	...
	for _, token := range []string{"workflow_preparations", "workflow_outbox", "workflow_input_resources", "driver_content", "chat_executor", "external_chat_run_events", "external_chat_hosts", "lease_token", "sub_agent_tasks", "sources", "plugin_step_intents"} {
