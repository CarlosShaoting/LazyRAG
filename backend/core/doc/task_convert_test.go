...
func TestNeedsOfficeConvertBeforeParse(t *testing.T) {
	...
	tests := []struct {
		...
		{"pptm with official mineru skips convert", pptmExt, officialMineruCfg, false},
...
func TestParsePathForIngestionMacroPresentationMineru(t *testing.T) {
	...
	cfg := map[string]any{"ocr_type": "mineru"}
	if got := parsePathForIngestion(d, cfg, documentParseProfileCloud); got != "/data/demo.pptm" {
		t.Fatalf("parsePathForIngestion() = %q, want original pptm path", got)
	}
}
