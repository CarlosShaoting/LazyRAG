...
def test_parse_workflow_markdown_bold():
    ...
    assert _parse_workflow(text) == 'KB_STYLE'
    assert _parse_workflow('WORKFLOW: FIND_AND_EDIT') == 'FIND_AND_EDIT'


def test_artifact_value_is_usable_image_url():
    from lazymind.chat.plugin.plugin_manager import _artifact_value_is_usable

    assert _artifact_value_is_usable(
        'generated_image_url',
        {'url': 'https://example.com/a.jpg'},
        'image',
    )
    assert not _artifact_value_is_usable(
        'generated_image_url',
...
def test_required_output_keys_kb_style_skips_existing_analyze_materials(loaded_plugin):
    from lazymind.chat.plugin.plugin_manager import _required_output_keys

    step_config = {
        'outputs': [
            {'artifact_id': 'material_summary', 'required': True},
            {'artifact_id': 'material_image', 'required': True},
        ],
    }
    cfg = {'plugin_session_id': 'sess-kb'}
    with patch(
        'lazymind.chat.plugin.plugin_manager._fetch_slot_artifact_text',
        return_value='**WORKFLOW**: KB_STYLE',
    ), patch(
        'lazymind.chat.plugin.plugin_manager._session_has_usable_artifact',
        side_effect=lambda _sid, key: key == 'material_summary',
    ):
        keys = _required_output_keys(step_config, 'collect_materials', cfg)
    assert 'material_summary' not in keys
    assert 'material_image' in keys
