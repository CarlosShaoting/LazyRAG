...
Route:
    POST /api/chat/generate_workflow   Generate workflow.yaml + state.yml from a description or skill content.
"""
...
# ---------------------------------------------------------------------------
# Workflow format spec — loaded once at module load time.
# ---------------------------------------------------------------------------


def _load_workflow_format_spec() -> str:
    """Load docs/workflow-format.md from the repository root."""
    candidates = [
        Path(__file__).parent.parent.parent.parent.parent.parent / 'docs' / 'workflow-format.md',
        Path('/app/docs/workflow-format.md'),
    ]
    for path in candidates:
        if path.exists():
            return path.read_text(encoding='utf-8')
    logger.warning('workflow-format.md not found; generating without spec')
    return ''


_PLUGIN_FORMAT_SPEC: str = _load_workflow_format_spec()

# ---------------------------------------------------------------------------
# Required field definitions — sync with docs/workflow-format.md when fields change.
# Each entry is a dot-separated path into the parsed dict (workflow.* or state.*).
# ---------------------------------------------------------------------------
...
_REQUIRED_STEP_FIELDS = ['id', 'label']           # workflow.yaml steps entries
...
_SYSTEM_PROMPT_TEMPLATE = (
    ...
    'Generate a valid LazyMind workflow consisting of exactly two YAML files:\n'
    '  1. workflow.yaml — workflow metadata and slot definitions (list format)\n'
    ...
    '- workflow.yaml slots must be a list of objects with {id, type, ...}; NOT a map.\n'
    ...
    '- Mark genuine extra user/session-provided materials with external: true in workflow.yaml slots.\n'
    ...
    '    {"workflow_yaml": "...", "state_yaml": "...", "scenario_md": "...", "scripts": {}}\n'
...
_USER_PROMPT_DESCRIPTION = 'Generate a workflow based on the following description:\n\n{description}'
_USER_PROMPT_SKILL = 'Convert the following skill content into a workflow:\n\n{skill_content}'

_PATCH_PROMPT_TEMPLATE = (
    'The previously generated workflow has the following missing or invalid fields:\n'
    '{missing_fields}\n\n'
    'Here is the current workflow state for context:\n'
    '--- workflow.yaml ---\n'
    '{workflow_yaml}\n'
    '--- state.yml ---\n'
    '{state_yaml}\n\n'
...
def _parse_initial_response(raw: str) -> Tuple[
    ...
    if not workflow_yaml or not state_yaml:
        raise ValueError(f'LLM response missing workflow_yaml or state_yaml: {list(data.keys())}')

    try:
        workflow_dict = yaml.safe_load(workflow_yaml) or {}
    except yaml.YAMLError as exc:
        raise ValueError(f'workflow_yaml is not valid YAML: {exc}') from exc
    ...
    return workflow_yaml, state_yaml, scenario_md, scripts, workflow_dict, state_dict
...
def _apply_patch(
    ...
    workflow_yaml_current = yaml.dump(workflow_dict, allow_unicode=True, sort_keys=False)
    ...
    patch_prompt = _PATCH_PROMPT_TEMPLATE.format(
        ...
        workflow_yaml=workflow_yaml_current,
...
# ---------------------------------------------------------------------------
# WorkflowSpec validation (reuses workflow_loader._validate logic)
# ---------------------------------------------------------------------------


def _validate_with_workflowspec(workflow_yaml: str, state_yaml: str) -> Optional[str]:
    """Write YAML to a temp dir and construct a WorkflowSpec to run _validate().

    Returns None on success, or an error message string on failure.
    """
    try:
        from lazymind.chat.workflow.workflow_loader import WorkflowSpec  # noqa: PLC0415
    except ImportError:
        logger.warning('WorkflowSpec not available; skipping runtime validation')
        return None

    with tempfile.TemporaryDirectory() as tmpdir:
        workflow_dir = Path(tmpdir) / '_gen_workflow'
        scenario_dir = workflow_dir / 'scenario'
        scenario_dir.mkdir(parents=True)

        (workflow_dir / 'workflow.yaml').write_text(workflow_yaml, encoding='utf-8')
        (scenario_dir / 'state.yml').write_text(state_yaml, encoding='utf-8')
        # scenario.md is required by WorkflowSpec; provide a minimal placeholder
        (scenario_dir / 'scenario.md').write_text('# placeholder', encoding='utf-8')

        try:
            WorkflowSpec(workflow_id='_gen_workflow', workflow_dir=workflow_dir)
            return None
...
def _generate_and_validate(
    ...
    """Run the full generate → field-check → patch loop → WorkflowSpec validate.

    Returns (workflow_yaml, state_yaml, scenario_md, scripts).
    Raises ValueError if all retries fail.
    """
    full_prompt = f'{system_prompt}\n\n{user_prompt}'
    raw = _call_llm(full_prompt)

    workflow_yaml, state_yaml, scenario_md, scripts, workflow_dict, state_dict = _parse_initial_response(raw)

    # --- Layer 1: required field path check + patch retry loop ---
    for attempt in range(MAX_PATCH_RETRIES):
        missing = _check_missing_fields(workflow_dict, state_dict)
        if not missing:
            break
        logger.info(
            '[generate_workflow] attempt=%d missing fields: %s', attempt + 1, missing
        )
        workflow_dict, state_dict = _apply_patch(workflow_dict, state_dict, missing, system_prompt)
    else:
    ...
    # --- Layer 2: WorkflowSpec runtime validation + patch retry ---
    for attempt in range(MAX_PATCH_RETRIES):
        error_msg = _validate_with_workflowspec(workflow_yaml, state_yaml)
        if error_msg is None:
            break
        logger.info(
            '[generate_workflow] WorkflowSpec validation error (attempt=%d): %s', attempt + 1, error_msg
        )
        # Build a pseudo missing-fields list from the error message so the same
        # patch mechanism can be reused.
        pseudo_missing = [f'WorkflowSpec validation error: {error_msg}']
        workflow_dict_patched, state_dict_patched = _apply_patch(
            workflow_dict, state_dict, pseudo_missing, system_prompt
        )
        workflow_yaml = yaml.dump(workflow_dict_patched, allow_unicode=True, sort_keys=False)
        state_yaml = yaml.dump(state_dict_patched, allow_unicode=True, sort_keys=False)
        workflow_dict, state_dict = workflow_dict_patched, state_dict_patched
    else:
        error_msg = _validate_with_workflowspec(workflow_yaml, state_yaml)
        if error_msg:
            raise ValueError(f'WorkflowSpec validation failed after {MAX_PATCH_RETRIES} retries: {error_msg}')

    return workflow_yaml, state_yaml, scenario_md, scripts
...
class GenerateWorkflowRequest(BaseModel):
    ...
    description: Optional[str] = Field(None, description='Natural-language description of the workflow goal')
...
@router.post(
    '/api/chat/generate_workflow',
    response_model=GenerateWorkflowResponse,
    summary='Generate workflow.yaml, state.yml, scenario.md and optional scripts from a description or skill content',
)
async def generate_workflow(req: GenerateWorkflowRequest) -> GenerateWorkflowResponse:
    """Synchronously generate a LazyMind workflow from a natural-language description or an existing skill.

    Called by the Go asyncjob worker (workflow_draft_generate).
    Returns workflow_yaml, state_yaml, scenario_md, and scripts.
    """
    inject_model_config(req.llm_config or {})

    session_id = f'generate_workflow_{req.name}'
    try:
    ...
    try:
        workflow_yaml, state_yaml, scenario_md, scripts = _generate_and_validate(
        ...
        logger.exception('Unexpected error during generate_workflow')
