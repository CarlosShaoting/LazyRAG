"""Staged workflow generation API routes.

Three sequential endpoints for phased workflow generation:

  POST /api/chat/generate_workflow/skeleton
      Phase 1 — Generate workflow.yaml skeleton (metadata + slots + steps, no step prompts).
      Returns: workflow_yaml (skeleton), step_ids list.

  POST /api/chat/generate_workflow/state_machine
      Phase 2 — Generate full state.yml (transitions + steps[].prompt).
      Input: workflow_yaml from phase 1.
      Returns: state_yaml.

  POST /api/chat/generate_workflow/scenario_scripts
      Phase 3 — Generate scenario.md and optional scripts/*.py.
      Input: workflow_yaml + state_yaml from phases 1 and 2.
...
from lazymind.chat.api.generate_workflow_routes import (
    _PLUGIN_FORMAT_SPEC,
    MAX_PATCH_RETRIES,
    _call_llm,
    _check_missing_fields,
    _validate_with_workflowspec,
    _extract_json,
)
...
_DESIGN_BRIEF_SYSTEM = (
    'You are a LazyMind workflow authoring assistant.\n'
    'Your task is Phase 0: produce a concise design brief for a workflow.\n\n'
    'Output a Markdown document with EXACTLY these three sections:\n\n'
    '## Workflow Overview\n'
    '(One paragraph: what this workflow does and what problem it solves.)\n\n'
    '## Slots\n'
...
_SKELETON_SYSTEM = (
    'You are a LazyMind workflow authoring assistant.\n'
    'Your task is Phase 1 of a 3-phase workflow generation: produce a workflow.yaml SKELETON.\n\n'
    ...
    '        file  + *       → file-card\n'
    '        json  + *       → json-block\n'
    '  - tool_scripts — ONLY include this section when the workflow genuinely needs custom Python\n'
    ...
    'Include every required field described above inside `workflow`. Do not return YAML and do not '
...
_STATE_MACHINE_SYSTEM = (
    'You are a LazyMind workflow authoring assistant.\n'
    'Your task is Phase 2 of a 3-phase workflow generation: produce a complete state.yml.\n\n'
    ...
    'CRITICAL RULE — every step listed in workflow.yaml MUST have a transitions entry\n'
    '(even if it only leads to __end__).\n\n'
    'The workflow.yaml skeleton from Phase 1 is provided below as context.\n'
    'Your step IDs in state.yml MUST match the steps in workflow.yaml exactly.\n\n'
    ...
    '{__spec__}\n'
    '=== End of Specification ===\n\n'
    '=== Workflow Skeleton (workflow.yaml from Phase 1) ===\n'
    '{__workflow_yaml__}\n'
    '=== End of Skeleton ===\n\n'
    '{__design_brief_section__}'
)
...
_SCENARIO_SCRIPTS_SYSTEM = (
    'You are a LazyMind workflow authoring assistant.\n'
    'Your task is Phase 3 of a 3-phase workflow generation:\n'
    '  1. Write scenario.md — a structured guide for the workflow editor.\n'
    ...
    '## 场景描述\n\n'
    '<One or two paragraphs describing what this workflow does and when to use it.>\n\n'
    '## 工作流程\n\n'
    '### {step_id}（{step_label}）\n\n'
    '<One or two sentences describing what this step does.>\n\n'
    '(repeat for every step in the same order as steps[] in workflow.yaml)\n\n'
    '## 注意事项\n\n'
    '<Optional usage tips, constraints, or warnings. Omit this section if nothing to add.>\n\n'
    'RULES:\n'
    '  - Use the EXACT step ids and labels from workflow.yaml steps[].\n'
    ...
    'WRITE scripts only when the workflow needs capabilities the LLM alone cannot perform:\n'
    ...
    '  - The workflow is a text/content generation pipeline\n\n'
    ...
    'workflow.yaml tool_scripts[].functions.\n'
...
def _check_skeleton_missing(workflow_dict: Dict[str, Any]) -> List[str]:
    missing: List[str] = []
    for field in _REQUIRED_SKELETON_PLUGIN_TOP:
        val = workflow_dict.get(field)
        if val is None or val == '' or val == [] or val == {}:
            missing.append(f'workflow.{field}')
    slots = workflow_dict.get('slots')
    if isinstance(slots, list):
        for i, slot in enumerate(slots):
            if not isinstance(slot, dict):
                missing.append(f'workflow.slots[{i}] (must be a dict)')
                continue
            for f in _REQUIRED_SKELETON_SLOT_FIELDS:
                if not slot.get(f):
                    missing.append(f'workflow.slots[{i}].{f}')
    steps = workflow_dict.get('steps')
    if isinstance(steps, list):
        for i, step in enumerate(steps):
            if not isinstance(step, dict):
                missing.append(f'workflow.steps[{i}] (must be a dict)')
                continue
            for f in _REQUIRED_SKELETON_STEP_FIELDS:
                if not step.get(f):
                    missing.append(f'workflow.steps[{i}].{f}')
    # Blocking: ui.tabs must be present for the frontend to render the layout
    ui = workflow_dict.get('ui')
    if not isinstance(ui, dict) or not ui.get('tabs'):
        missing.append('workflow.ui.tabs (ui tabs block missing, frontend cannot render layout)')
    elif isinstance(ui.get('tabs'), list):
        placed: set[str] = set()
        for i, tab in enumerate(ui['tabs']):
            if not isinstance(tab, dict):
                missing.append(f'workflow.ui.tabs[{i}] (must be a dict)')
                continue
            tab_slots = tab.get('slots')
            if not isinstance(tab_slots, list) or not tab_slots:
                missing.append(f'workflow.ui.tabs[{i}].slots (must contain at least one declared slot)')
                continue
        ...
        for slot_id in sorted(declared - placed):
            missing.append(f'workflow.ui.tabs slot coverage (declared slot {slot_id!r} is not placed in any tab)')
    return missing


_SKELETON_PATCH_TEMPLATE = (
    'The generated workflow.yaml skeleton has missing fields:\n'
    '{__missing_fields__}\n\n'
    'Current skeleton:\n{__workflow_yaml__}\n\n'
    'Return ONLY a JSON patch: {"workflow": {...}}\n'
    'Fix only the missing fields. No explanation.'
)

_SKELETON_YAML_REPAIR_TEMPLATE = (
    'The workflow skeleton below was returned as YAML, but it is invalid and could not be parsed.\n'
    ...
    'YAML parser error:\n{__yaml_error__}\n\n'
    'Invalid skeleton:\n{__workflow_yaml__}\n\n'
    'Return ONLY: {"workflow": {<complete workflow skeleton>}}'
)
...
def _patch_skeleton(
    ...
    workflow_yaml_str = yaml.dump(workflow_dict, allow_unicode=True, sort_keys=False)
    patch_prompt = _tmpl(
        _SKELETON_PATCH_TEMPLATE,
        missing_fields='\n'.join(f'  - {f}' for f in missing),
        workflow_yaml=workflow_yaml_str,
    )
    ...
    if 'workflow' in patch and isinstance(patch['workflow'], dict):
        from lazymind.chat.api.generate_workflow_routes import _deep_merge  # noqa: PLC0415
        workflow_dict = _deep_merge(workflow_dict, patch['workflow'])
    return workflow_dict


def _workflow_dict_from_skeleton_response(
    data: Dict[str, Any],
    system_prompt: str,
) -> Dict[str, Any]:
    ...
    if not isinstance(workflow_yaml, str) or not workflow_yaml.strip():
        raise HTTPException(status_code=500, detail='Phase 1: missing workflow object in response')
    try:
        ...
        parsed = repaired.get('workflow')
        if not isinstance(parsed, dict):
            raise HTTPException(
                status_code=500,
                detail=f'Phase 1 YAML parse error: {exc}; repair response missing workflow object',
            ) from exc
    if not isinstance(parsed, dict):
        raise HTTPException(status_code=500, detail='Phase 1: workflow skeleton must be an object')
    return parsed
...
def _dry_run_import(filename: str, source: str) -> Optional[str]:
    ...
    """
    with tempfile.TemporaryDirectory(prefix='lazymind_workflow_dryrun_') as tmpdir:
        script_path = Path(tmpdir) / Path(filename).name
        script_path.write_text(source, encoding='utf-8')
        module_name = f'_workflow_dryrun_{script_path.stem}'
...
_NODE_FIX_TEMPLATE = (
    ...
    '- If an existing query-like pseudo-slot has no real producer, remove its workflow.yaml slot definition and all '
...
def _validate_slot_references(
    ...
    """Check that every slot id referenced in state.yml is defined in workflow.yaml.

    Returns a list of error strings, empty when all references are valid.
    """
    defined_slots: set = {
        s.get('id')
        for s in (workflow_dict.get('slots') or [])
...
def _validate_material_semantics(
    ...
    external_slots = {
        str(slot.get('id'))
        for slot in (workflow_dict.get('slots') or [])
...
def _validate_material_contract(
    ...
    return [
        *_validate_slot_references(workflow_dict, state_dict),
        *_validate_material_semantics(workflow_dict, state_dict),
    ]


_SLOT_REPAIR_SYSTEM = (
    'You are a workflow schema doctor. You receive a workflow.yaml (slots definition) and a '
    'state.yml (inputs/outputs/route/skip expressions) that have mismatched slot IDs.\n\n'
    'Your task: fix the mismatch. You may EITHER:\n'
    '  A) Add missing slot definitions to workflow.yaml slots[] when the state.yml references '
    ...
    'declared in workflow.yaml slots[], when the state.yml used wrong / inconsistent IDs.\n'
    '  C) Do both, when appropriate.\n\n'
    'Rules:\n'
    '- Prefer renaming state.yml references to match existing workflow.yaml slot IDs whenever '
    'a clear semantic match exists (same concept, different name).\n'
    '- Only add new slot entries to workflow.yaml when there is genuinely new data being '
    'produced with no equivalent in the existing slots.\n'
    '- Do NOT remove any existing slot from workflow.yaml.\n'
    ...
    '- Return ONLY valid JSON: {"workflow_yaml": "...", "state_yaml": "..."}\n'
...
def _repair_slots_only(
    ...
    """Use LLM to fix slot-reference mismatches between workflow.yaml and state.yml.

    The LLM decides whether to add missing slots to workflow.yaml, rename wrong
    references in state.yml, or both.  Returns (fixed_workflow_dict, fixed_state_dict).
    ...
    user_prompt = (
        ...
        f'Current state.yml:\n{state_yaml_str}\n\n'
        'Fix the slot mismatches and return the corrected workflow.yaml and state.yml as JSON.'
    )
    ...
    try:
        ...
        return workflow_dict, state_dict

    fixed_workflow_dict = workflow_dict
    fixed_state_dict = state_dict

    if new_workflow_yaml := data.get('workflow_yaml', ''):
        try:
            parsed = yaml.safe_load(new_workflow_yaml)
            if isinstance(parsed, dict):
                fixed_workflow_dict = parsed
        except yaml.YAMLError as exc:
            logger.warning('[staged/slot_repair] workflow_yaml parse failed: %s', exc)
    ...
    remaining = _validate_slot_references(fixed_workflow_dict, fixed_state_dict)
    logger.info(
        '[staged/slot_repair] after LLM repair: %d remaining errors (was %d)',
        len(remaining), len(errors),
    )
    return fixed_workflow_dict, fixed_state_dict
...
class StateMachineRequest(_LLMConfigMixin):
    name: str
    workflow_yaml: str  # output from Phase 1
    design_brief: Optional[str] = None
    workflow_analysis: Optional[str] = None
...
class StateMachineResponse(BaseModel):
    state_yaml: str
    workflow_yaml: str = ''  # updated when slot repair was applied
    warnings: List[str] = []
...
def _apply_tool_replacements(
    ...
    if mappings:
        workflow['required_framework_tools'] = sorted({replacement for _, replacement, _ in mappings})
    ...
    try:
        ...
        pass
    if skipped_paths and isinstance(workflow.get('tool_scripts'), list):
        workflow['tool_scripts'] = [
            entry for entry in workflow['tool_scripts']
...
@router.post('/api/chat/generate_workflow/analyze_skill', response_model=AnalyzeSkillResponse)
...
@router.post(
    '/api/chat/generate_workflow/design_brief',
...
@router.post(
    '/api/chat/generate_workflow/skeleton',
    response_model=SkeletonResponse,
    summary='Phase 1: Generate workflow.yaml skeleton',
)
async def generate_skeleton(req: SkeletonRequest) -> SkeletonResponse:
    """Phase 1: generate workflow.yaml skeleton (slots + steps list, no state logic)."""
    inject_model_config(req.llm_config or {})
    ...
    if package_prompt.strip():
        ...
        user_prompt = (
            f'Workflow name: {req.name}\n\n'
            f'Convert the following versioned skill package into a workflow skeleton. '
            f'Do not invent behavior for unresolved files. '
            f'Confirmed workflow:\n{workflow}\n\n{package_prompt}'
        )
    else:
        user_prompt = (
            f'Workflow name: {req.name}\n\n'
            f'Generate a workflow skeleton based on the following description:\n\n'
            f'{req.description or req.name}'
        )
    ...
    workflow_dict = _workflow_dict_from_skeleton_response(data, system_prompt)
    _apply_tool_replacements(workflow_dict, None, req.workflow_analysis)

    # Validate skeleton fields + patch retry
    for attempt in range(MAX_PATCH_RETRIES):
        missing = _check_skeleton_missing(workflow_dict)
        ...
        workflow_dict = _patch_skeleton(workflow_dict, blocking, system_prompt)
    else:
        missing = _check_skeleton_missing(workflow_dict)
        blocking = [m for m in missing if 'warn:' not in m]
        if blocking:
    ...
    slots_list = workflow_dict.get('slots', [])
    if isinstance(slots_list, list):
        ui = workflow_dict.setdefault('ui', {})
    ...
    workflow_yaml = yaml.dump(workflow_dict, allow_unicode=True, sort_keys=False)
    return SkeletonResponse(workflow_yaml=workflow_yaml)


@router.post(
    '/api/chat/generate_workflow/state_machine',
    response_model=StateMachineResponse,
    summary='Phase 2: Generate state.yml from workflow.yaml skeleton',
)
async def generate_state_machine(req: StateMachineRequest) -> StateMachineResponse:
    """Phase 2: generate full state.yml using the workflow.yaml skeleton from Phase 1."""
    inject_model_config(req.llm_config or {})

    try:
        workflow_dict = yaml.safe_load(req.workflow_yaml) or {}
    except yaml.YAMLError as exc:
    ...
    user_prompt = (
        f'Workflow name: {req.name}\n\nSteps in order: '
        f'{", ".join(s.get("id", "") for s in workflow_dict.get("steps", []) if isinstance(s, dict)) or "(none)"}\n\n'
        'Generate the complete state.yml for this workflow, including transitions and step prompts.\n'
    ...
    for attempt in range(MAX_PATCH_RETRIES):
        # Reuse _check_missing_fields but only for state fields (pass empty workflow_dict)
        missing = _check_missing_fields({}, state_dict)
        # Filter to state.* only since workflow is already done
        ...
        state_dict = _patch_state_machine(workflow_dict, state_dict, state_missing, system_prompt)
    else:
    ...
    # WorkflowSpec validation (both YAML together)
    state_yaml = yaml.dump(state_dict, allow_unicode=True, sort_keys=False)
    for attempt in range(MAX_PATCH_RETRIES):
        error_msg = _validate_with_workflowspec(req.workflow_yaml, state_yaml)
        if error_msg is None:
            break
        logger.info('[staged/state_machine] WorkflowSpec error (attempt=%d): %s', attempt + 1, error_msg)
        state_dict = _patch_state_machine(
            workflow_dict, state_dict,
            [f'WorkflowSpec validation error: {error_msg}'],
            system_prompt,
        )
        state_yaml = yaml.dump(state_dict, allow_unicode=True, sort_keys=False)
    else:
        err = _validate_with_workflowspec(req.workflow_yaml, state_yaml)
        if err:
            warn_msg = f'Phase 2 WorkflowSpec validation failed: {err}'
            logger.warning('[staged/state_machine] degraded: %s', warn_msg)
            field_warnings.append(warn_msg)

    # Slot reference validation + repair (max 1 attempt).
    # Run after WorkflowSpec so slot errors are as accurate as possible.
    slot_errors = _validate_slot_references(workflow_dict, state_dict)
    if slot_errors:
        logger.info('[staged/state_machine] slot errors: %s', slot_errors)
        workflow_dict, state_dict = _repair_slots_only(workflow_dict, state_dict, slot_errors, req.llm_config or {})
        # Re-validate; if still broken, log a warning (non-fatal).
        remaining_slot_errors = _validate_slot_references(workflow_dict, state_dict)
    ...
    _apply_tool_replacements(workflow_dict, state_dict, req.workflow_analysis)

    final_workflow_yaml = yaml.dump(workflow_dict, allow_unicode=True, sort_keys=False)
    state_yaml = yaml.dump(state_dict, allow_unicode=True, sort_keys=False)
    return StateMachineResponse(state_yaml=state_yaml, workflow_yaml=final_workflow_yaml, warnings=field_warnings)


@router.post(
    '/api/chat/generate_workflow/scenario_scripts',
...
async def generate_scenario_scripts(req: ScenarioScriptsRequest) -> ScenarioScriptsResponse:
    ...
    user_prompt = (
        f'Workflow name: {req.name}\n\n'
        'Write scenario.md for this workflow. '
...
# ---------------------------------------------------------------------------
# Workflow info polish endpoint
# ---------------------------------------------------------------------------

_POLISH_FIELD_INSTRUCTIONS: Dict[str, str] = {
    'description': (
        'Polish this workflow description to be concise and professional. '
        'Clearly describe what the workflow does in 1-2 sentences. '
...
_POLISH_SYSTEM = (
    'You are a professional editor for AI workflow documentation. '
...
@router.post(
    '/api/chat/generate_workflow/polish_info',
    response_model=PolishInfoResponse,
    summary='Polish workflow info fields with AI',
)
async def polish_workflow_info(req: PolishInfoRequest) -> PolishInfoResponse:
    """Polish one or more workflow info fields (description, when_to_use, overview, notes).
...
_REPAIR_SYSTEM = (
    ...
    'You are given:\n'
    '  - The current workflow.yaml (for context: step IDs, slot IDs, etc.)\n'
    ...
    'CRITICAL RULE — transitions.__start__ MUST always be present and non-empty.\n'
    'CRITICAL RULE — every step in workflow.yaml MUST have a transitions entry.\n'
    'CRITICAL RULE — every step in workflow.yaml MUST have a prompt in state.yml.\n\n'
    'MATERIAL SEMANTICS:\n'
    ...
    'user_query, search_query, request, topic, task_description, or instructions from workflow.yaml and remove their '
    ...
    '  {"workflow_yaml": "<complete corrected workflow.yaml>", '
...
class RepairResponse(BaseModel):
    state_yaml: str
    workflow_yaml: str = ''  # populated when slot repair was applied
...
@router.post(
    '/api/chat/generate_workflow/repair',
...
async def repair_state_machine(req: RepairRequest) -> RepairResponse:
    ...
    Accepts the current workflow.yaml and state.yml, plus an optional user hint.
    Returns a fully corrected state.yml.
    """
    logger.info(
        '[repair] START target=%r repair_hint=%r warnings=%s '
        'workflow_yaml_len=%d state_yaml_len=%d',
        req.target,
        (req.repair_hint or '')[:120],
        req.warnings,
        len(req.workflow_yaml),
        len(req.state_yaml),
    )
    inject_model_config(req.llm_config or {})
    ...
    try:
        ...
        logger.error('[repair] invalid workflow_yaml: %s', exc)
        raise HTTPException(status_code=400, detail=f'Invalid workflow_yaml: {exc}') from exc
    ...
    if req.target == 'scripts':
        ...
        repaired_workflow, repaired_state = workflow_dict, state_dict
        if ignored_paths and isinstance(repaired_workflow.get('tool_scripts'), list):
            repaired_workflow['tool_scripts'] = [
                entry for entry in repaired_workflow['tool_scripts']
        ...
        return RepairResponse(
            ...
            workflow_yaml=yaml.dump(repaired_workflow, allow_unicode=True, sort_keys=False),
    ...
    if req.target == 'scenario':
        scenario_system = (
            'You are a LazyMind workflow authoring assistant.\n'
            'Your task is to write or repair the scenario.md for a workflow.\n\n'
            'scenario.md is a structured guide for the workflow editor.\n\n'
            '=== scenario.md FORMAT (STRICTLY REQUIRED) ===\n'
            'The scenario.md MUST follow this exact Markdown structure (write in Chinese):\n\n'
            '## 场景描述\n\n'
            '<One or two paragraphs describing what this workflow does and when to use it.>\n\n'
        ...
        scenario_user = f'{hint_section}Write a complete scenario.md for this workflow.'
    ...
    if req.target == 'ui':
        ui_system = (
            'You are repairing the UI layout in a LazyMind workflow.yaml.\n'
            'Return the COMPLETE workflow.yaml, preserving all non-UI behavior and identifiers.\n'
            ...
            'Return ONLY JSON: {"workflow_yaml": "<complete fixed workflow.yaml>"}.\n'
        )
        ...
        try:
            data = _extract_json(raw)
            fixed_workflow_yaml = str(data.get('workflow_yaml') or '')
            fixed_workflow = yaml.safe_load(fixed_workflow_yaml) or {}
        except (ValueError, yaml.YAMLError) as exc:
            raise HTTPException(status_code=500, detail=f'UI repair parse error: {exc}') from exc
        if not fixed_workflow_yaml or not isinstance(fixed_workflow, dict):
            raise HTTPException(status_code=500, detail='UI repair: missing complete workflow_yaml')

        remaining = [m for m in _check_skeleton_missing(fixed_workflow) if m.startswith('workflow.ui.')]
        for _ in range(MAX_PATCH_RETRIES):
            if not remaining:
                break
            fixed_workflow = _patch_skeleton(fixed_workflow, remaining, ui_system)
            remaining = [m for m in _check_skeleton_missing(fixed_workflow) if m.startswith('workflow.ui.')]
        fixed_workflow_yaml = yaml.dump(fixed_workflow, allow_unicode=True, sort_keys=False)
        logger.info('[repair/ui] SUCCESS workflow_yaml_len=%d remaining=%s', len(fixed_workflow_yaml), remaining)
        return RepairResponse(
            state_yaml=req.state_yaml,
            workflow_yaml=fixed_workflow_yaml,
            scenario_md=req.scenario_md,
            scripts=req.scripts,
    ...
    pre_slot_errors = _validate_material_contract(workflow_dict, state_dict)
    ...
    if pre_slot_errors:
        slot_error_section = (
            'SLOT REFERENCE ERRORS (must fix — use ONLY the slot ids defined in workflow.yaml slots[]):\n'
    ...
    repaired_workflow_yaml = ''
    candidate_workflow_yaml = data.get('workflow_yaml', '')
    if candidate_workflow_yaml:
        try:
            candidate_workflow_dict = yaml.safe_load(candidate_workflow_yaml) or {}
            if isinstance(candidate_workflow_dict, dict):
                workflow_dict = candidate_workflow_dict
                repaired_workflow_yaml = yaml.dump(workflow_dict, allow_unicode=True, sort_keys=False)
        except yaml.YAMLError as exc:
            logger.warning('[repair/statemachine] ignoring invalid workflow_yaml from repair: %s', exc)
    ...
    # Slot check before structural/WorkflowSpec retries: only used to update remaining.
    # The definitive slot repair runs after all retries (see below) so that structural
    # or WorkflowSpec retries cannot re-introduce slot errors undetected.
    # Structural pre-check before WorkflowSpec: catch common LLM mistakes that produce
    # cryptic AttributeErrors inside WorkflowSpec (e.g. transitions generated as a list).
    ...
    if struct_err:
        ...
        try:
            ...
            candidate_workflow_yaml2 = data2.get('workflow_yaml', '')
            if candidate_workflow_yaml2:
                candidate_workflow_dict2 = yaml.safe_load(candidate_workflow_yaml2) or {}
                if isinstance(candidate_workflow_dict2, dict):
                    workflow_dict = candidate_workflow_dict2
                    repaired_workflow_yaml = yaml.dump(workflow_dict, allow_unicode=True, sort_keys=False)
    ...
    # Final full validation — must pass WorkflowSpec before we call this a success.
    # Failure here triggers one more LLM retry with the WorkflowSpec error injected.
    final_workflow_yaml_for_check = repaired_workflow_yaml if repaired_workflow_yaml else req.workflow_yaml
    workflowspec_err = _validate_with_workflowspec(final_workflow_yaml_for_check, fixed_yaml_out)
    if workflowspec_err:
        logger.warning('[repair/statemachine] WorkflowSpec failed, requesting LLM retry: %s', workflowspec_err)
        retry_prompt = (
            f'{warnings_section}'
        ...
        try:
            ...
            candidate_workflow_yaml3 = data3.get('workflow_yaml', '')
            if candidate_workflow_yaml3:
                candidate_workflow_dict3 = yaml.safe_load(candidate_workflow_yaml3) or {}
                if isinstance(candidate_workflow_dict3, dict):
                    workflow_dict = candidate_workflow_dict3
                    repaired_workflow_yaml = yaml.dump(workflow_dict, allow_unicode=True, sort_keys=False)
                    final_workflow_yaml_for_check = repaired_workflow_yaml
            ...
            logger.error('[repair/statemachine] WorkflowSpec retry parse failed: %s', exc)
        workflowspec_err2 = _validate_with_workflowspec(final_workflow_yaml_for_check, fixed_yaml_out)
        if workflowspec_err2:
            logger.warning('[repair/statemachine] WorkflowSpec still fails after retry: %s', workflowspec_err2)
            raise HTTPException(
                status_code=422,
                detail=f'Repair produced invalid YAML (WorkflowSpec): {workflowspec_err2}',
            )
        logger.info('[repair/statemachine] WorkflowSpec retry succeeded')

    # ── Final slot reference check + repair ──────────────────────────────────
    # Run this AFTER all structural/WorkflowSpec retries so that any retry that
    ...
    # exclusively in workflow.yaml; the frontend parser ignores them in state.yml but
    # they are confusing and can mask V8 validation errors if left in.
    fixed_dict.pop('slots', None)
    fixed_yaml_out = yaml.dump(fixed_dict, allow_unicode=True, sort_keys=False)
    slot_errors = _validate_material_contract(workflow_dict, fixed_dict)
    for repair_attempt in range(MAX_PATCH_RETRIES):
        if not slot_errors:
            break
        logger.info('[repair/statemachine] slot errors on final state: %s', slot_errors)
        workflow_dict, fixed_dict = _repair_slots_only(workflow_dict, fixed_dict, slot_errors, req.llm_config or {})
        fixed_dict.pop('slots', None)
        fixed_yaml_out = yaml.dump(fixed_dict, allow_unicode=True, sort_keys=False)
        slot_errors = _validate_material_contract(workflow_dict, fixed_dict)
        logger.info(
    ...
    logger.info(
        '[repair/statemachine] SUCCESS state_yaml_len=%d workflow_yaml_updated=%s remaining=%s',
        len(fixed_yaml_out),
        bool(repaired_workflow_yaml),
        remaining,
    )
    return RepairResponse(state_yaml=fixed_yaml_out, workflow_yaml=repaired_workflow_yaml, remaining_warnings=remaining)
