...
def find_producer_steps(plugin_id: str, artifact_id: str) -> List[str]:
    """Return all step_ids that can produce artifact_id, preserving state.yml order."""
    spec = get_plugin(plugin_id)
    if not spec:
        return []
    producers: List[str] = []
...
def find_producer_step(plugin_id: str, artifact_id: str) -> Optional[str]:
    """Return one step_id that produces artifact_id, or None."""
    producers = find_producer_steps(plugin_id, artifact_id)
