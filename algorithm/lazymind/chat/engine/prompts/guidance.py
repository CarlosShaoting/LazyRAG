...
ATTACHED_IMAGES_GUIDANCE = (
    ...
    '1. **Edit / modify** the attachment → call `image_editor` immediately with `url` = a '
    'listed path and `prompt` = the edit instruction. Do NOT call `describe_attached_images` '
    'first.\n'
    '2. **New image from text** (no attachment edit) → call `image_generator`.\n'
    '3. **KB file name / metadata** → call `kb_search`.\n'
    '4. **Describe content only** → call `describe_attached_images` or `vision_extractor` '
    'with a listed path.\n'
)
