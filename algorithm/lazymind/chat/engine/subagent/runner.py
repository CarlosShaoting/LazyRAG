...
def _format_kb_prefetch_block(payload: Any) -> str:
    ...
    if isinstance(result, dict):
        ...
        items = result
    lines = [
        '## Knowledge base prefetch (authoritative)',
        '',
...
def _prefetch_kb_on_runner_thread(
    ctx: 'SubAgentContext',
    tool_names: Optional[List[str]],
) -> str:
    """Run kb_search in a clean globals session, then restore the SubAgent session.

    SubAgent task sessions (and StreamCallHelper workers) hit
    ``Attr pickled_data not found in globals`` on UrlModule RPCs. A fresh
    minimal session can call kb_search successfully; results are injected into
    the objective so the in-worker agent never needs the kb tool.
    """
    if not tool_names:
        return ''
    normalized = {str(t).strip().lower() for t in tool_names}
    ...
    filters = dict(ctx.params.get('filters') or agentic.get('filters') or {})
    kb_id = filters.get('kb_id')
    if isinstance(kb_id, list):
        kb_id = next((str(x).strip() for x in kb_id if str(x).strip()), '')
    else:
    ...
    filters['kb_id'] = kb_id
    query = str(ctx.params.get('user_input') or ctx.objective or '').strip()
    if not query:
        return ''
    user_id = str(ctx.params.get('user_id') or agentic.get('user_id') or '').strip()

    import uuid
    prev_g_sid = lazyllm.globals._sid
    prev_l_sid = lazyllm.locals._sid
    probe_sid = f'kb-prefetch-{uuid.uuid4().hex}'
    try:
        from lazymind.chat.engine.tools.kb import KBToolGroup
        # Fresh session: avoids broken pickled_data on the SubAgent task sid.
        lazyllm.globals._init_sid(sid=probe_sid)
        lazyllm.locals._init_sid(sid=probe_sid)
        ...
        _ = lazyllm.globals.pickled_data  # fail fast if session is unusable
        group = KBToolGroup()
        group._ensure_search_runtime()
        payload = group.kb_search(query=query, filters=filters)
        block = _format_kb_prefetch_block(payload)
        LOG.info(
            '[SubAgent] KB prefetched on runner thread query=%s hits_block_len=%s',
            query[:80], len(block),
        )
        return block
    except Exception as exc:
        LOG.warning('[SubAgent] KB runner-thread prefetch failed: %s', exc)
        return ''
    finally:
        lazyllm.globals._init_sid(sid=prev_g_sid)
        lazyllm.locals._init_sid(sid=prev_l_sid)
...
async def run_subagent_stream(
    ...
    try:
        ...
        kb_block = _prefetch_kb_on_runner_thread(ctx, tools)
        if kb_block:
            ctx.objective = (ctx.objective or '').rstrip() + '\n\n' + kb_block
        if had_kb_tool:
            # Worker-thread kb_search always fails; never expose the tool there.
            tools = [t for t in (tools or []) if str(t).strip().lower() != 'kb']
