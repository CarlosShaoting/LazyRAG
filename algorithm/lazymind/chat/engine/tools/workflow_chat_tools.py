...
Provides create_workflow_draft: a three-step workflow that creates a workflow draft
via the Go core API and triggers AI generation.

Workflow (driven by create-workflow SKILL.md):
  Step 1 — LLM produces a natural-language workflow summary.
  Step 2 — ask_user confirms the plan (ask_user is a separate stop-tool).
  Step 3 — User confirms → call create_workflow_draft to create, pre-fill, and
...
def create_workflow_draft(
    ...
    """Create a new workflow draft and trigger AI generation.

    Call this tool ONLY after the user has explicitly confirmed the workflow plan
    (confirmed via ask_user with type='boolean').

    This tool performs three sequential Go core API calls:
      1. POST /workflow-drafts — creates a blank draft, obtains draft_id.
      2. POST /workflow-drafts/{id}:save — pre-writes the confirmed name and
         description so the AI generation uses them as constraints.
      3. POST /workflow-drafts/{id}:ai-generate — triggers the three-phase async
         generation job.

    After the tool returns, write a short confirmation message that includes
    a Markdown link to the workflow editor using the editor_url in the result.

    Args:
        name: Workflow display name (e.g. "合同审阅助手").
        description: One-paragraph description of what the workflow does.
    ...
    Returns a dict with:
        ...
        editor_url  — Relative URL to open the workflow editor, e.g. /workflow/<id>.
    ...
    try:
        create_resp = post_core_api('/workflow-drafts', {'name': name})
    except RuntimeError as exc:
        return tool_error(f'Failed to create workflow draft: {exc}')
    ...
    if not draft_id:
        return tool_error('Failed to create workflow draft: no draft_id returned')
    ...
    try:
        ...
        post_core_api(f'/workflow-drafts/{draft_id}:save', {
            'workflow_yaml_content': skeleton_yaml,
            'version': 0,
        })
    except Exception as exc:  # noqa: BLE001
        # Pre-fill failure is non-fatal; generation can still proceed
        _write_agent_data({'workflow_draft_prefill_warning': str(exc)})

    # Step 3: Trigger async AI generation
    try:
        post_core_api(f'/workflow-drafts/{draft_id}:ai-generate', {'description': full_description})
    except RuntimeError as exc:
        return tool_error(f'Failed to trigger AI generation: {exc}')

    editor_url = f'/workflow/{draft_id}'

    # Emit workflow_draft_created SSE event so future frontend cards can pick it up
    _write_agent_data({
