...
def _annotate_query_with_attached_images(query: str, image_files: list[str]) -> str:
    seen: set[str] = set()
    ...
    for path in image_files:
        ...
        if not value:
            continue
        local_path = resolve_local_image_path(value) or value
    ...
    lines = [
        ...
        '1) Edit/modify → `image_editor` with `url` below; do NOT call '
        '`describe_attached_images` first.',
...
async def handle_chat(query: str, history: Optional[List[Dict[str, Any]]],
    ...
    other_files, image_files = split_image_files(resolved_files)
    ...
    agent_query = _annotate_query_with_attached_images(query, image_files)
    translator = AgentEventFrameTranslator(query=agent_query)
